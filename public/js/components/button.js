...
var ICON_COLORS = {
  ...
  copy:   { bg: 'bg-violet-100', hover: 'hover:bg-violet-200', text: 'text-violet-600' },
  plan:   { bg: 'bg-yellow-100', hover: 'hover:bg-yellow-200', text: 'text-yellow-600' }
};
...
var ICON_SVG = {
  ...
  plan: '<svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>'
};
...
export function iconButtonHtml(type, tooltip, attrs, tooltipPos) {
  ...
  var base = (tooltipPos === 'below' || tooltipPos === 'below-left') ? 'top-full mt-2 ' : 'bottom-full mb-2 ';
  var pos = tooltipPos === 'right'
    ? 'right-0 origin-bottom-right'
    : tooltipPos === 'below'
      ? 'left-1/2 -translate-x-1/2 origin-top'
      : tooltipPos === 'below-left'
        ? 'left-0 origin-top'
  ...
  return '<div class="relative group">' +
    '<button class="' + c.bg + ' ' + c.hover + ' ' + c.text + ' p-2 rounded-xl transition' + extraClass + '"' + attrStr + '>' +
      svg +
    '</button>' +
    '<span class="absolute ' + base + 'scale-0 group-hover:scale-100 ' + pos + ' transition-transform duration-200 bg-slate-900 text-white text-xs px-3 py-2 rounded-lg whitespace-nowrap shadow-lg border border-slate-600 z-50">' +
      escapeHtml(tooltip) +
    '</span>' +
  '</div>';
}

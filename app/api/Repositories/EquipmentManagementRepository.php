...
class EquipmentManagementRepository extends BaseRepository
{
    ...
    public function count(?string $search): int
    {
        if ($search) {
            ...
            $stmt = $this->safePrepare("SELECT COUNT(*) AS total FROM equipamentos");
        }
        $stmt->execute();
        $result = $stmt->get_result();
    ...
    {
        $stmt = $this->safePrepare("DELETE FROM equipamentos WHERE id = ?");
        $stmt->bind_param('i', $id);
        $stmt->execute();
        $stmt->close();
    }
    ...
    {
        $stmt = $this->safePrepare("SELECT COUNT(*) AS total FROM registros WHERE equipamento_id = ?");
        $stmt->bind_param('i', $id);
        $stmt->execute();
        $result = $stmt->get_result();
        ...
        $stmt = $this->safePrepare("SELECT COUNT(*) AS total FROM pv WHERE equipamento_id = ?");
        $stmt->bind_param('i', $id);
        $stmt->execute();
        $result = $stmt->get_result();
    ...
    {
        $stmt = $this->safePrepare("SELECT endereco_id FROM equipamentos WHERE id = ?");

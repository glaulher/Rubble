...
def _arg_flag(nome):
    return nome in sys.argv[1:]


def _arg_valor(nome, padrao=""):
    args = sys.argv[1:]
    for i, arg in enumerate(args):
        if arg == nome and i + 1 < len(args):
            return args[i + 1]
        prefixo = nome + "="
        if arg.startswith(prefixo):
            return arg[len(prefixo):]
    return padrao


MODO_SERVIDOR = _arg_flag("--server") or _arg_flag("--servidor") or str(os.environ.get("TEMPO_FECHADO_MODO_SERVIDOR", "")).strip().lower() in {"1", "true", "sim", "server", "servidor"}
HOST = _arg_valor("--host", os.environ.get("TEMPO_FECHADO_HOST", "0.0.0.0" if MODO_SERVIDOR else "127.0.0.1"))
PORT = int(_arg_valor("--port", os.environ.get("TEMPO_FECHADO_PORT", "5050")) or 5050)
CHECK_HOST = "127.0.0.1"
BROWSER_HOST = _arg_valor("--browser-host", os.environ.get("TEMPO_FECHADO_BROWSER_HOST", "127.0.0.1"))
URL = f"http://{BROWSER_HOST}:{PORT}"
URL_ACESSO_REDE = f"http://{socket.gethostname()}:{PORT}" if MODO_SERVIDOR else URL
ABRIR_NAVEGADOR = (not MODO_SERVIDOR) or _arg_flag("--open-browser")
DATA_DIR_ARG = _arg_valor("--data-dir", os.environ.get("TEMPO_FECHADO_PONTO_PDFS_DIR", ""))
if DATA_DIR_ARG:
    os.environ["TEMPO_FECHADO_PONTO_PDFS_DIR"] = DATA_DIR_ARG
if MODO_SERVIDOR:
    os.environ["TEMPO_FECHADO_MODO_SERVIDOR"] = "1"
os.environ["TEMPO_FECHADO_PORT"] = str(PORT)
os.environ["TEMPO_FECHADO_HOST"] = str(HOST)
...
def avisar_instancia_antiga_v8215(info):
    try:
        ...
        mensagem = (
            "Ja existe uma instancia do Tempo Fechado aberta na porta 5050.\n\n"
            f"Launcher atual: {APP_FULL_NAME}\n"
            f"Instancia encontrada: {detalhe or 'nao identificada'}\n\n"
            "Se a tela continuar mostrando versao antiga, feche o Tempo Fechado, finalize processos "
        ...
        avisar_erro_sem_console("Tempo Fechado - versao ativa", mensagem)
    except Exception:
        pass
...
def iniciar_servidor():
    try:
        ...
        log(f"Iniciando Flask em host={HOST} porta={PORT} modo={'servidor' if MODO_SERVIDOR else 'local'} ponto_pdfs={ponto_pdfs_base()}")
...
def main():
    try:
        if executar_script_embarcado_v82124():
            return
        log("Inicializando Tempo Fechado v8.21.67 - Horas Extras Simplificadas...")
        log(f"Modo={'servidor' if MODO_SERVIDOR else 'local'} | host={HOST} | porta={PORT} | ponto_pdfs={ponto_pdfs_base()}")
        garantir_ponto_pdfs()
        ...
        if porta_aberta():
            ...
            modo_ativo = str(info.get("modo_operacao") or "").strip().lower() if isinstance(info, dict) else ""
            modo_desejado = "servidor" if MODO_SERVIDOR else "local"
            if versao_ativa == APP_VERSION:
                if modo_ativo == modo_desejado:
                    log(f"Instancia atual ja ativa em {URL}: {info.get('titulo') or versao_ativa} | modo={modo_ativo}")
                    if ABRIR_NAVEGADOR:
                        webbrowser.open(URL)
                    else:
                        log(f"Modo servidor ativo. Acesso na rede: {URL_ACESSO_REDE}")
                    return
                log(f"Instancia {versao_ativa} ativa em modo {modo_ativo or 'desconhecido'}, mas o modo desejado e {modo_desejado}. Reiniciando instancia.")
        ...
        if aguardar():
            if ABRIR_NAVEGADOR:
                webbrowser.open(URL)
            log(f"Tempo Fechado iniciado em modo {'servidor' if MODO_SERVIDOR else 'local'}. URL local: {URL}. URL rede: {URL_ACESSO_REDE}. Logs em {log_file_path()}.")

...
PERFIS_VALIDOS = {"admin", "supervisor", "operador", "consulta"}


def _default_users_from_env():
    raw = _os.environ.get("TEMPO_FECHADO_DEFAULT_USERS_JSON", "").strip()
    if raw:
        try:
...
# Hardcoded defaults removidos — senhas devem vir de TEMPO_FECHADO_DEFAULT_USERS_JSON (.env)
# Ex: TEMPO_FECHADO_DEFAULT_USERS_JSON='{"admin":{"senha":"...","nome":"Admin","perfil":"admin"}}'
USUARIOS_PADRAO = _default_users_from_env()
...
def inicializar_banco_usuarios(db_path: Path, legacy_usuarios: Optional[dict] = None) -> None:
    ...
    with _conectar(db_path) as conn:
        ...
        conn.execute("UPDATE usuarios SET perfil = 'consulta' WHERE COALESCE(perfil, '') NOT IN ('admin','supervisor','operador','consulta')")
        ...
        if total_admin_ativo == 0 and "admin" in USUARIOS_PADRAO:
...
def criar_ou_atualizar_usuario(db_path: Path, usuario: str, nome: str, perfil: str, senha: str = "", ativo: bool = True, trocar_senha: bool = True) -> Tuple[bool, str]:
    ...
    if perfil not in PERFIS_VALIDOS:
        return False, "Perfil invalido. Use admin, supervisor, operador ou consulta."

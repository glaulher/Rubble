...
from flask import Flask, render_template, jsonify, request, send_file, redirect, url_for, session, has_request_context
import base64
import gzip
...
def _modo_servidor_ativo_v82139():
    return str(os.environ.get("TEMPO_FECHADO_MODO_SERVIDOR", "")).strip().lower() in {"1", "true", "sim", "server", "servidor"}


def _ponto_pdfs_base_v82139():
    configurado = str(os.environ.get("TEMPO_FECHADO_PONTO_PDFS_DIR", "") or "").strip().strip('"').strip("'")
    if configurado:
        return Path(os.path.expandvars(configurado)).expanduser()
    return Path.home() / "ponto_pdfs"
...
app = Flask(__name__)
# Secret movido para .env (TDD secrets) — nunca hardcodado. Fallback só em dev.
_secret_from_env = (
    os.environ.get("TEMPO_FECHADO_SECRET_KEY")
    or os.environ.get("FLASK_SECRET_KEY")
    or os.environ.get("JWT_SECRET")
)
if _secret_from_env:
    app.secret_key = _secret_from_env
elif os.environ.get("APP_DEBUG", "").lower() == "true":
    app.secret_key = "dev-only-fallback-do-not-use-in-prod"
else:
    raise RuntimeError("TEMPO_FECHADO_SECRET_KEY ausente — defina no .env (use JWT_SECRET)")
app.config["SESSION_COOKIE_HTTPONLY"] = True
app.config["SESSION_COOKIE_SAMESITE"] = "Lax"
app.config["SESSION_COOKIE_SECURE"] = True
app.config["MAX_CONTENT_LENGTH"] = 250 * 1024 * 1024

# Traefik termina TLS e repassa via HTTP; sem ProxyFix request.is_secure seria sempre False
try:
...
APP_VERSION = "v8.21.67"
APP_RELEASE_NAME = "Horas Extras Simplificadas"
APP_FULL_NAME = "Tempo Fechado v8.21.67 - Horas Extras Simplificadas"
...
USER_CONFIG_DIR = _ponto_pdfs_base_v82139() / "config"
...
ARQUIVO_CONFIG_EMAIL_ROBO_V81727 = USER_CONFIG_DIR / "config_email_robo.json"
...
ARQUIVO_ANOTACOES_OPERACIONAIS = DATA_DIR / "anotacoes_operacionais.json"
...
ANOTACOES_OPERACIONAIS_LOCK = threading.Lock()
...
ARQUIVO_TRAVA_PROCESSAMENTO_V82141 = DATA_DIR / "processamento_trava_operacional.json"
ARQUIVO_HISTORICO_FILA_V82141 = DATA_DIR / "processamento_fila_historico.jsonl"
PROCESSAMENTO_INTEGRADO_LOCK = threading.Lock()
# v8.21.49 - Trava unica persistente para execucoes pesadas no modo central.
PROCESSAMENTO_OPERACIONAL_LOCK = threading.Lock()
PROCESSAMENTO_OPERACIONAL_PERSISTENCIA_CARREGADA = False
PROCESSAMENTO_OPERACIONAL_TRAVA = {
    "ativa": False,
    "id": "",
    "status": "livre",
    "tipo": "",
...
ARQUIVO_EXCEL_ROBO_PADRAO = _ponto_pdfs_base_v82139() / "saida" / "resultado_lote_consultas_validado.xlsx"
...
def _carregar_anotacoes_operacionais_v82150():
    try:
        if not ARQUIVO_ANOTACOES_OPERACIONAIS.exists():
            return {}
        with ARQUIVO_ANOTACOES_OPERACIONAIS.open("r", encoding="utf-8") as f:
            dados = json.load(f)
...
def _salvar_anotacoes_operacionais_v82150(dados):
    DATA_DIR.mkdir(parents=True, exist_ok=True)
    tmp = ARQUIVO_ANOTACOES_OPERACIONAIS.with_suffix(".json.tmp")
    with tmp.open("w", encoding="utf-8") as f:
        json.dump(dados if isinstance(dados, dict) else {}, f, ensure_ascii=False, indent=2, sort_keys=True)
    tmp.replace(ARQUIVO_ANOTACOES_OPERACIONAIS)


def _limpar_texto_anotacao_v82150(valor, limite=600):
...
def validar_jwt_rubble(token):
    try:
        ...
        if cabecalho.get("alg") != JWT_ALGORITHM:
            return None, "Algoritmo JWT nao permitido."
        # Strict: só aceita iss == rubble (JWT do Rubble tem iss implícito via username/role, mas exige exp e assinatura válida)
        iss = str(payload.get("iss") or "").strip().lower()
        # Rubble JWT (JwtHelper.php) não seta iss; aceita vazio apenas se tem username/role e user_id
        if iss not in ("", "rubble"):
            return None, "Emissor JWT invalido."
        if iss == "" and not (payload.get("username") and payload.get("role") and payload.get("user_id") is not None):
            return None, "Emissor JWT invalido."
        if int(payload.get("exp") or 0) < int(time.time()):
            return None, "Token expirado."
        # Rubble payload: username, nome, role; fallback para sub
        usuario_raw = str(payload.get("username") or payload.get("sub") or "").strip()
        if not usuario_raw:
            return None, "Usuario ausente no token."
        usuario = usuario_raw.lower()
        nome = str(payload.get("nome") or payload.get("name") or usuario_raw).strip() or usuario_raw
        role = str(payload.get("role") or payload.get("perfil") or "consulta").strip().lower()
        perfil = RUBBLE_ROLE_TO_TF_PERFIL.get(role, "consulta")
        # Sincroniza usuário local sem exigir senha (trocar_senha=False)
        dados_exist = obter_usuario(ARQUIVO_USUARIOS_DB, usuario)
        if not dados_exist:
            ok, msg = criar_ou_atualizar_usuario(
                ARQUIVO_USUARIOS_DB, usuario, nome, perfil, senha="", ativo=True, trocar_senha=False
            )
            if not ok:
                return None, msg or "Falha ao sincronizar usuario."
            dados_exist = obter_usuario(ARQUIVO_USUARIOS_DB, usuario)
        else:
            # Atualiza nome/perfil se mudou no Rubble (sem mexer na senha)
            if dados_exist.get("nome") != nome or dados_exist.get("perfil") != perfil:
                criar_ou_atualizar_usuario(
                    ARQUIVO_USUARIOS_DB, usuario, nome, perfil, senha="", ativo=True, trocar_senha=bool(dados_exist.get("trocar_senha"))
                )
                dados_exist = obter_usuario(ARQUIVO_USUARIOS_DB, usuario)
        if not dados_exist or not dados_exist.get("ativo"):
            return None, "Usuario do token nao encontrado ou desativado."
        return {
            "usuario": dados_exist.get("usuario"),
            "nome": dados_exist.get("nome"),
            "perfil": dados_exist.get("perfil", perfil),
            "trocar_senha": bool(dados_exist.get("trocar_senha", False)),
            "jwt_payload": payload,
            "origem": "rubble_sso",
        }, ""
    except Exception as e:
...
def autenticar_requisicao_atual():
    ...
    # Query ?token= só é aceita no endpoint /sso (evita vazamento em access_log para /api/*)
    if not token and request.path == "/sso":
    ...
    if token:
        ...
        if dados:
            ...
            return dados, ""
        # Fallback: tenta validar como JWT do Rubble (mesmo secret compartilhado)
        dados_r, erro_r = validar_jwt_rubble(token)
        if dados_r:
            _aplicar_usuario_na_sessao(dados_r)
            return dados_r, ""
        # Se falhou nos dois, usa erro do último para API
        erro = erro_r or erro
...
PERMISSOES_OPERACIONAIS_V82143 = {
    "consultar": {"admin", "supervisor", "operador", "consulta"},
    "upload_pdfs": {"admin", "supervisor", "operador"},
    "processar": {"admin", "supervisor"},
    "administrar": {"admin"},
}


ROTULOS_PERFIL_V82143 = {
    "admin": "Administrador",
    "supervisor": "Supervisor",
    "operador": "Operador",
    "consulta": "Consulta",
...
def _perfil_normalizado_v82143():
    perfil = str(perfil_usuario() or "consulta").strip().lower()
    return perfil if perfil in ROTULOS_PERFIL_V82143 else "consulta"


def usuario_tem_permissao_v82143(permissao):
    perfil = _perfil_normalizado_v82143()
    return perfil in PERMISSOES_OPERACIONAIS_V82143.get(permissao, {"admin"})


def permissoes_usuario_atual_v82143(perfil=None):
    perfil = str(perfil or _perfil_normalizado_v82143()).strip().lower()
    if perfil not in ROTULOS_PERFIL_V82143:
        perfil = "consulta"
    return {
        "consultar_dashboards": True,
        "baixar_excel": True,
        "upload_pdfs": perfil in PERMISSOES_OPERACIONAIS_V82143["upload_pdfs"],
        "processar": perfil in PERMISSOES_OPERACIONAIS_V82143["processar"],
        "importar_excel": perfil in PERMISSOES_OPERACIONAIS_V82143["processar"],
        "enviar_alertas": perfil == "admin",
        "gerenciar_usuarios": perfil == "admin",
        "administrar": perfil == "admin",
    }
...
def sso_rubble():
    ...
    dados, erro = validar_jwt_rubble(token)
    if not dados:
        # tenta também como token TF nativo para compatibilidade
        dados, erro2 = validar_jwt_usuario(token)
        if not dados:
            registrar_auditoria("SSO_NEGADO", erro or erro2 or "token invalido", "NEGADO")
            return redirect(url_for("login"))
    _aplicar_usuario_na_sessao(dados)
    registrar_auditoria("SSO_RUBBLE", f"SSO via Rubble para {dados.get('usuario')}", "OK")
    resp = redirect(url_for("index"))
    return _resposta_com_cookie_jwt(resp, dados)
...
def permissao_operacional_v82143(permissao, descricao="esta acao"):
    def decorator(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            dados_auth, erro_auth = autenticar_requisicao_atual()
            if not dados_auth:
                if request.path.startswith("/api/"):
                    return jsonify({"ok": False, "erro": erro_auth or "Sessao expirada. Faca login novamente."}), 401
                return redirect(url_for("login"))
            if not usuario_tem_permissao_v82143(permissao):
                registrar_auditoria("ACESSO_NEGADO", f"{request.path} | permissao={permissao}", "NEGADO")
                perfil = _perfil_normalizado_v82143()
                permitidos = sorted(PERMISSOES_OPERACIONAIS_V82143.get(permissao, {"admin"}))
                mensagem = f"Acesso negado. O perfil {ROTULOS_PERFIL_V82143.get(perfil, perfil)} nao pode executar {descricao}."
                if request.path.startswith("/api/"):
                    return jsonify({
                        "ok": False,
                        "erro": mensagem,
                        "perfil_atual": perfil,
                        "perfis_permitidos": permitidos,
                    }), 403
                return mensagem, 403
            return func(*args, **kwargs)
        return wrapper
...
def api_versao_publica_v8215():
    return jsonify({
        ...
        "modo_operacao": "servidor" if _modo_servidor_ativo_v82139() else "local",
        "ponto_pdfs_base": str(_ponto_pdfs_base_v82139()),
    })
...
@app.route("/api/anotacoes-operacionais", methods=["GET"])
@login_obrigatorio
def api_anotacoes_operacionais_v82150():
    with ANOTACOES_OPERACIONAIS_LOCK:
        dados = _carregar_anotacoes_operacionais_v82150()
    return jsonify({"ok": True, "anotacoes": dados})


@app.route("/api/anotacoes-operacionais", methods=["POST"])
@login_obrigatorio
def api_salvar_anotacao_operacional_v82150():
    payload = request.get_json(silent=True) or {}
    chave = _limpar_texto_anotacao_v82150(payload.get("chave"), 260)
    if not chave:
        return jsonify({"ok": False, "erro": "Chave da anotacao nao informada."}), 400
...
def api_usuario_atual():
    perfil = _perfil_normalizado_v82143()
    permissoes = permissoes_usuario_atual_v82143(perfil)
    return jsonify({
        "usuario": session.get("usuario"),
        "nome": session.get("nome"),
        "perfil": perfil,
        "perfil_rotulo": ROTULOS_PERFIL_V82143.get(perfil, perfil),
...
def _periodo_fechamento_v82166(valor):
    texto = str(valor or "").strip()
    if not texto:
        texto = datetime.now().strftime("%Y-%m")
    if not re.match(r"^\d{4}-\d{2}$", texto):
        raise ValueError("Informe o periodo no formato AAAA-MM.")
    ano, mes = texto.split("-")
    if not (1 <= int(mes) <= 12):
        raise ValueError("Mes invalido no periodo informado.")
    return f"{ano}-{mes}"


def _copiar_arquivo_fechamento_v82166(origem, destino_base, categoria, itens, erros):
    try:
        src = Path(origem)
        ...
        destino = destino_base / src.name
        if destino.exists():
            destino = destino_base / f"{src.stem}_{int(time.time())}{src.suffix}"
        shutil.copy2(src, destino)
        tamanho = int(destino.stat().st_size)
        itens.append({"categoria": categoria, "origem": str(src), "destino": str(destino), "bytes": tamanho})
    except Exception as e:
        erros.append({"categoria": categoria, "origem": str(origem), "erro": str(e)})


def _copiar_padroes_fechamento_v82166(pasta, padroes, destino_base, categoria, itens, erros, limite=2000):
    try:
        raiz = Path(pasta)
        ...
        total = 0
        for padrao in padroes:
            for item in raiz.glob(padrao):
                if item.is_file():
                    _copiar_arquivo_fechamento_v82166(item, destino_base, categoria, itens, erros)
                    total += 1
                    if total >= limite:
                        return
    except Exception as e:
        erros.append({"categoria": categoria, "origem": str(pasta), "erro": str(e)})


def _executar_fechamento_periodo_v82166(payload=None):
    payload = payload or {}
    periodo = _periodo_fechamento_v82166(payload.get("periodo"))
    observacao = str(payload.get("observacao") or "").strip()[:600]
    base = _ponto_pdfs_base_v82139()
    destino = base / "backup" / "fechamentos" / f"{periodo}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    itens = []
    erros = []
    destino.mkdir(parents=True, exist_ok=True)

    dados_destino = destino / "base_tempo_fechado"
    for arquivo in [
        ARQUIVO_PADRAO,
        ARQUIVO_BASE_ATIVA_FLAG,
        ARQUIVO_SALDOS_BASE_MEDICAO,
        ...
        ARQUIVO_RESUMO_BANCO_HORAS_PDF,
        ARQUIVO_CACHE_BH2_CALCULO,
        ARQUIVO_ANOTACOES_OPERACIONAIS,
        ARQUIVO_HISTORICO_OPERACAO_V8190,
        ARQUIVO_HISTORICO_FILA_V82141,
        ARQUIVO_LOG_PROCESSAMENTO_INTEGRADO,
    ]:
        _copiar_arquivo_fechamento_v82166(arquivo, dados_destino, "base_tempo_fechado", itens, erros)
    ...
    pastas = {
        ...
        "erro": ["*.pdf", "*.PDF"],
        "saida": ["*.xlsx", "*.xls", "*.csv"],
        "controle": ["*.log", "*.json", "*.jsonl", "*.txt"],
        "config": ["*.json", "*.ini", "*.txt"],
    }
    for nome, padroes in pastas.items():
        _copiar_padroes_fechamento_v82166(raiz / nome, padroes, destino / "ponto_pdfs" / nome, f"ponto_pdfs_{nome}", itens, erros)

    manifesto = {
        "ok": not bool(erros),
        "periodo": periodo,
        "gerado_em": _agora_iso_v8190(),
        "versao": APP_VERSION,
        "modo_operacao": "servidor" if _modo_servidor_ativo_v82139() else "local",
        "observacao": observacao,
        "ponto_pdfs_base": str(base),
        "raiz_robo": str(raiz),
        "destino": str(destino),
        "resumo": {
            "arquivos": len(itens),
            "bytes": sum(int(x.get("bytes") or 0) for x in itens),
            "erros": len(erros),
        },
        "itens": itens,
        "erros": erros,
    }
    manifesto_path = destino / "manifesto_fechamento.json"
    manifesto_path.write_text(json.dumps(manifesto, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
    itens.append({"categoria": "manifesto", "origem": "", "destino": str(manifesto_path), "bytes": int(manifesto_path.stat().st_size)})
    manifesto["itens"] = itens
    manifesto["resumo"]["arquivos"] = len(itens)
    manifesto["resumo"]["bytes"] = sum(int(x.get("bytes") or 0) for x in itens)
    manifesto_path.write_text(json.dumps(manifesto, ensure_ascii=False, indent=2, default=str), encoding="utf-8")

    mensagem = f"Fechamento {periodo} arquivado com {len(itens)} arquivo(s)."
    registrar_historico_operacao_v8190(
        "Fechamento do Periodo",
        "OK" if not erros else "PARCIAL",
        mensagem,
        {"periodo": periodo, "destino": str(destino), "arquivos": len(itens), "erros": len(erros)},
        origem="Processamento",
    )
    manifesto.update({"mensagem": mensagem, "parcial": bool(erros), "erro": ""})
    return manifesto
...
def api_cleanup_operacional_v82117():
    ok_trava, trava = _adquirir_trava_processamento_v82140(
        "Cleanup Operacional",
        "Processamento",
        "Cleanup operacional em andamento.",
    )
    if not ok_trava:
        return _resposta_trava_ocupada_v82140(trava)
    status_trava = "concluido"
    mensagem_trava = "Cleanup operacional finalizado."
    try:
        ...
        if status_code >= 400 or not resultado.get("ok", True):
            status_trava = "erro"
            mensagem_trava = resultado.get("erro") or "Cleanup operacional finalizado com erro."
        else:
            status_trava = "concluido_com_aviso" if resultado.get("parcial") else "concluido"
            mensagem_trava = resultado.get("mensagem") or mensagem_trava
        return jsonify(resultado), status_code
    except Exception as e:
        status_trava = "erro"
        mensagem_trava = f"Falha no cleanup operacional: {e}"
        raise
    finally:
        _liberar_trava_processamento_v82140(trava.get("id"), status_trava, mensagem_trava)


@app.route("/api/processamento/fechamento-periodo", methods=["POST"])
@login_obrigatorio
@admin_obrigatorio
def api_processamento_fechamento_periodo_v82166():
    ok_trava, trava = _adquirir_trava_processamento_v82140(
        "Fechamento do Periodo",
        "Processamento",
        "Fechamento do periodo em andamento.",
    )
    if not ok_trava:
        return _resposta_trava_ocupada_v82140(trava)
    status_trava = "concluido"
    mensagem_trava = "Fechamento do periodo finalizado."
    try:
        resultado = _executar_fechamento_periodo_v82166(request.get_json(silent=True) or {})
        status_trava = "concluido_com_aviso" if resultado.get("parcial") else "concluido"
        mensagem_trava = resultado.get("mensagem") or mensagem_trava
        return jsonify(resultado)
    except Exception as e:
        status_trava = "erro"
        mensagem_trava = f"Falha no fechamento do periodo: {e}"
        LOGGER.exception(mensagem_trava)
        return jsonify({"ok": False, "erro": str(e), "mensagem": mensagem_trava}), 500
    finally:
        _liberar_trava_processamento_v82140(trava.get("id"), status_trava, mensagem_trava)
...
def api_dashboard_banco_he():
    ...
    # v8.21.39: prioriza resposta imediata a partir do resumo ja salvo.
    ...
    # v8.21.39: Dias com HE e Dias com ausência passam a ser métrica real.
...
def _status_saude_operacional_v8190(eventos):
    ...
    base = Path(cfg.get("pasta_pdfs_robo") or _ponto_pdfs_base_v82139())
    entrada = base / "entrada"
    saida = base / "saida"
    processados = base / "processados"
    erro = base / "erro"
    excel = _normalizar_path_windows(cfg.get("arquivo_excel_robo"), _excel_robo_padrao_v81737()) if "_normalizar_path_windows" in globals() else Path(cfg.get("arquivo_excel_robo") or _ponto_pdfs_base_v82139() / "saida" / "resultado_lote_consultas_validado.xlsx")
    ...
    if launcher.exists():
        add_diag("Coleta integrada", "OK", f"Disponível · atualizado há {_idade_arquivo_v8191(launcher) or '-'}", "ok")
    else:
        add_diag("Coleta integrada", "Erro", "Componente de coleta não encontrado.", "danger")
        add_alerta("alto", "Coleta integrada indisponível", "O componente configurado para a coleta não foi encontrado.")
...
def _gerar_alertas_inteligentes_v8192(saude, eventos):
    ...
    for a in saude.get("alertas", []) or []:
        ...
        if "entrada" in tl or "aguardando" in tl:
            ...
            acao = "Conferir a configuração da coleta na Administração do Robô."
...
# ============================================================
# v8.21.39 - Painel de Implantacao / Log Amigavel
# ============================================================
...
# ============================================================
# v8.21.39 - Painel de Implantacao
# ============================================================
...
def _launcher_sem_pdfs_v82151(proc):
    texto = f"{getattr(proc, 'stdout', '') or ''}\n{getattr(proc, 'stderr', '') or ''}".lower()
    sinais = [
        "nenhum pdf foi baixado do outlook e a pasta entrada esta vazia",
        "nenhum pdf foi baixado do outlook e a pasta entrada está vazia",
        "total de pdfs baixados: 0",
        "total de pdfs compativeis encontrados: 0",
    ]
...
def _job_processamento_integrado_atual():
    ...
    payload["trava_operacional"] = _trava_operacional_atual_v82140()
    payload["historico_fila"] = _ler_historico_fila_v82141(12)
    return payload


def _usuario_operacao_atual_v82140():
    if not has_request_context():
        return {"usuario": "sistema", "nome": "Sistema", "perfil": ""}
    usuario = session.get("usuario") or session.get("login") or session.get("username") or ""
    perfil = session.get("perfil") or session.get("role") or ""
    nome = session.get("nome") or usuario
    return {"usuario": str(usuario or ""), "nome": str(nome or usuario or ""), "perfil": str(perfil or "")}


def _trava_operacional_padrao_v82141():
    return {
        "ativa": False,
        "id": "",
        "status": "livre",
        ...
        "liberado_em": "",
        "persistida_em": "",
        "recuperada_apos_reinicio": False,
    }


def _persistir_trava_processamento_v82141(trava):
    try:
        DATA_DIR.mkdir(parents=True, exist_ok=True)
        payload = _trava_operacional_padrao_v82141()
        payload.update(trava or {})
        payload["persistida_em"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        tmp = ARQUIVO_TRAVA_PROCESSAMENTO_V82141.with_suffix(f".{uuid.uuid4().hex}.tmp")
        tmp.write_text(json.dumps(payload, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
        tmp.replace(ARQUIVO_TRAVA_PROCESSAMENTO_V82141)
    except Exception:
        pass


def _registrar_historico_fila_v82141(evento, status, mensagem="", trava=None):
    try:
        DATA_DIR.mkdir(parents=True, exist_ok=True)
        item = {
            "quando": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "evento": str(evento or "fila"),
            "status": str(status or "INFO"),
            "mensagem": str(mensagem or "")[:1000],
            "trava": dict(trava or {}),
        }
        with ARQUIVO_HISTORICO_FILA_V82141.open("a", encoding="utf-8") as f:
            f.write(json.dumps(item, ensure_ascii=False, default=str) + "\n")
    except Exception:
        pass


def _ler_historico_fila_v82141(limite=20):
    try:
        if not ARQUIVO_HISTORICO_FILA_V82141.exists():
            return []
        linhas = ARQUIVO_HISTORICO_FILA_V82141.read_text(encoding="utf-8", errors="ignore").splitlines()
        eventos = []
        for linha in linhas[-max(int(limite or 20), 1):]:
            try:
                eventos.append(json.loads(linha))
            except Exception:
                continue
...
def _carregar_trava_persistida_v82141():
    try:
        if not ARQUIVO_TRAVA_PROCESSAMENTO_V82141.exists():
            return _trava_operacional_padrao_v82141()
        dados = json.loads(ARQUIVO_TRAVA_PROCESSAMENTO_V82141.read_text(encoding="utf-8", errors="ignore") or "{}")
        trava = _trava_operacional_padrao_v82141()
        if isinstance(dados, dict):
            trava.update(dados)
        if trava.get("ativa"):
            trava["recuperada_apos_reinicio"] = True
            trava["status"] = trava.get("status") or "rodando"
            if "recuperad" not in str(trava.get("mensagem") or "").lower():
                trava["mensagem"] = (str(trava.get("mensagem") or "Processamento em andamento.") + " Estado recuperado apos reinicio.").strip()
        return trava
    except Exception:
        return _trava_operacional_padrao_v82141()


def _sincronizar_trava_persistida_v82141():
    global PROCESSAMENTO_OPERACIONAL_PERSISTENCIA_CARREGADA
    if PROCESSAMENTO_OPERACIONAL_PERSISTENCIA_CARREGADA:
        return
    recuperada = None
    with PROCESSAMENTO_OPERACIONAL_LOCK:
        if PROCESSAMENTO_OPERACIONAL_PERSISTENCIA_CARREGADA:
            return
        trava = _carregar_trava_persistida_v82141()
        PROCESSAMENTO_OPERACIONAL_TRAVA.update(trava)
        PROCESSAMENTO_OPERACIONAL_PERSISTENCIA_CARREGADA = True
        if trava.get("ativa"):
            recuperada = dict(trava)
    if recuperada:
        _registrar_historico_fila_v82141("recuperacao", "ATIVA", "Trava operacional recuperada apos reinicio do servidor.", recuperada)


def _trava_operacional_atual_v82140():
    _sincronizar_trava_persistida_v82141()
    with PROCESSAMENTO_OPERACIONAL_LOCK:
        return dict(PROCESSAMENTO_OPERACIONAL_TRAVA)


def _adquirir_trava_processamento_v82140(tipo, origem, mensagem=""):
    _sincronizar_trava_persistida_v82141()
    agora = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    usuario = _usuario_operacao_atual_v82140()
    with PROCESSAMENTO_OPERACIONAL_LOCK:
        if PROCESSAMENTO_OPERACIONAL_TRAVA.get("ativa"):
            trava_ocupada = dict(PROCESSAMENTO_OPERACIONAL_TRAVA)
            _registrar_historico_fila_v82141("bloqueio", "BLOQUEADO", "Tentativa bloqueada por processamento em andamento.", trava_ocupada)
            return False, trava_ocupada
        PROCESSAMENTO_OPERACIONAL_TRAVA.update({
            "ativa": True,
            "id": uuid.uuid4().hex,
            "status": "rodando",
            "tipo": str(tipo or "Processamento"),
            "origem": str(origem or "Tempo Fechado"),
            "usuario": usuario.get("usuario", ""),
            "nome": usuario.get("nome", ""),
            "perfil": usuario.get("perfil", ""),
            "maquina": os.environ.get("COMPUTERNAME") or socket.gethostname(),
            "iniciado_em": agora,
            "atualizado_em": agora,
            "finalizado_em": "",
            "mensagem": mensagem or "Processamento em andamento.",
            "liberado_por": "",
            "liberado_em": "",
            "recuperada_apos_reinicio": False,
        })
        trava = dict(PROCESSAMENTO_OPERACIONAL_TRAVA)
    _persistir_trava_processamento_v82141(trava)
    _registrar_historico_fila_v82141("inicio", "RODANDO", mensagem or "Processamento iniciado.", trava)
    return True, trava


def _liberar_trava_processamento_v82140(lock_id=None, status="concluido", mensagem=""):
    _sincronizar_trava_persistida_v82141()
    agora = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    usuario = _usuario_operacao_atual_v82140()
    with PROCESSAMENTO_OPERACIONAL_LOCK:
        if not PROCESSAMENTO_OPERACIONAL_TRAVA.get("ativa"):
            trava_livre = dict(PROCESSAMENTO_OPERACIONAL_TRAVA)
            _persistir_trava_processamento_v82141(trava_livre)
            return trava_livre
        if lock_id and PROCESSAMENTO_OPERACIONAL_TRAVA.get("id") != lock_id:
            return dict(PROCESSAMENTO_OPERACIONAL_TRAVA)
        PROCESSAMENTO_OPERACIONAL_TRAVA.update({
            "ativa": False,
            "status": str(status or "concluido"),
            "atualizado_em": agora,
            "finalizado_em": agora,
            "mensagem": mensagem or "Processamento finalizado.",
            "liberado_por": usuario.get("usuario", ""),
            "liberado_em": agora,
            "recuperada_apos_reinicio": False,
        })
        trava = dict(PROCESSAMENTO_OPERACIONAL_TRAVA)
    _persistir_trava_processamento_v82141(trava)
    _registrar_historico_fila_v82141("fim", str(status or "concluido").upper(), mensagem or "Processamento finalizado.", trava)
    return trava


def _resposta_trava_ocupada_v82140(trava):
    nome = trava.get("nome") or trava.get("usuario") or "outro usuario"
    tipo = trava.get("tipo") or "processamento"
    inicio = trava.get("iniciado_em") or "-"
    return jsonify({
        "ok": False,
        "status": "bloqueado",
        "erro": f"Ja existe {tipo} em andamento por {nome} desde {inicio}. Aguarde concluir ou libere a trava pela Administracao.",
        "mensagem": "Processamento bloqueado por trava operacional ativa.",
        "trava_operacional": trava,
    }), 409


def _ip_rede_servidor_v82142():
    try:
...
def _ponto_pdfs_servidor_padrao_v82154():
    configurado = str(os.environ.get("TEMPO_FECHADO_PONTO_PDFS_DIR", "") or "").strip().strip('"').strip("'")
    if configurado:
        return Path(os.path.expandvars(configurado)).expanduser()
    return Path.home() / "TempoFechadoServidor" / "ponto_pdfs"


def _aspas_powershell_v82154(valor):
    return "'" + str(valor).replace("'", "''") + "'"


def _aspas_cmd_v82158(valor):
    return '"' + str(valor).replace('"', '""') + '"'


def _agendar_reinicio_modo_servidor_v82154():
    if _modo_servidor_ativo_v82139():
        return {
            "ok": True,
            "erro": "",
            "mensagem": "O Tempo Fechado ja esta em modo servidor.",
            "ja_ativo": True,
            "info": _info_servidor_assistido_v82142(),
        }

    try:
        porta = int(os.environ.get("TEMPO_FECHADO_PORT", "5050") or 5050)
    except Exception:
        porta = 5050

    base = _ponto_pdfs_servidor_padrao_v82154()
    for nome in ["entrada", "saida", "processados", "logs", "backup", "erro", "config", "controle", "backup_atualizacao"]:
        (base / nome).mkdir(parents=True, exist_ok=True)

    exe = Path(sys.executable).resolve()
    if not getattr(sys, "frozen", False):
        candidato = Path(APP_BASE_DIR) / "TempoFechado.exe"
        if candidato.exists():
            exe = candidato.resolve()
    ...
    if sys.platform.startswith("win"):
        helper = base / "controle" / "reiniciar_tempo_fechado_servidor.cmd"
        helper.parent.mkdir(parents=True, exist_ok=True)
        comando_exe = " ".join([_aspas_cmd_v82158(str(exe))] + [_aspas_cmd_v82158(a) for a in args])
        helper.write_text(
            "\r\n".join([
                "@echo off",
                "setlocal",
                "timeout /t 2 /nobreak >nul",
                f"taskkill /PID {pid_atual} /F >nul 2>nul",
                "timeout /t 1 /nobreak >nul",
                'set "TEMPO_FECHADO_MODO_SERVIDOR=1"',
                'set "TEMPO_FECHADO_HOST=0.0.0.0"',
                f'set "TEMPO_FECHADO_PORT={porta}"',
                f'set "TEMPO_FECHADO_PONTO_PDFS_DIR={str(base)}"',
                f"cd /d {_aspas_cmd_v82158(str(exe.parent))}",
                f"start \"Tempo Fechado Servidor\" {comando_exe}",
                "endlocal",
            ]) + "\r\n",
            encoding="utf-8",
        )
        ...
        subprocess.Popen([str(exe)] + args, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, stdin=subprocess.DEVNULL)

    ip_rede = _ip_rede_servidor_v82142()
    url_local = f"http://127.0.0.1:{porta}"
    url_rede = f"http://{ip_rede}:{porta}"
    registrar_historico_operacao_v8190(
        "Servidor Assistido",
        "OK",
        "Reinicio em modo servidor agendado pela interface.",
        {"exe": str(exe), "ponto_pdfs_base": str(base), "porta": porta, "url_local": url_local, "url_rede": url_rede, "helper": str(helper) if sys.platform.startswith("win") else ""},
        origem="Servidor Assistido",
    )
    return {
        "ok": True,
        "erro": "",
        "mensagem": "O Tempo Fechado vai fechar a instancia local e reabrir em modo Servidor.",
        "ja_ativo": False,
        "ponto_pdfs_base": str(base),
        "url_local": url_local,
        "teste_local_url": url_local.rstrip("/") + "/saude-servidor",
        "url_rede": url_rede,
        "teste_acesso_url": url_rede.rstrip("/") + "/saude-servidor",
    }


def _agendar_reinicio_modo_local_v82165():
    if not _modo_servidor_ativo_v82139():
        return {
            "ok": True,
            "erro": "",
            "mensagem": "O Tempo Fechado ja esta em modo local.",
            "ja_ativo": True,
            "info": _info_servidor_assistido_v82142(),
        }
    ...
    pid_atual = os.getpid()
    base_controle = _ponto_pdfs_base_v82139() / "controle"
    helper = None
    if sys.platform.startswith("win"):
        helper = base_controle / "reiniciar_tempo_fechado_local.cmd"
        helper.parent.mkdir(parents=True, exist_ok=True)
        comando_exe = " ".join([_aspas_cmd_v82158(str(exe))] + [_aspas_cmd_v82158(a) for a in args])
        helper.write_text(
            "\r\n".join([
                ...
                f"taskkill /PID {pid_atual} /F >nul 2>nul",
                "timeout /t 1 /nobreak >nul",
                'set "TEMPO_FECHADO_MODO_SERVIDOR="',
                'set "TEMPO_FECHADO_HOST=127.0.0.1"',
                f'set "TEMPO_FECHADO_PORT={porta}"',
                'set "TEMPO_FECHADO_PONTO_PDFS_DIR="',
                f"cd /d {_aspas_cmd_v82158(str(exe.parent))}",
                f"start \"Tempo Fechado Local\" {comando_exe}",
                "endlocal",
            ]) + "\r\n",
            encoding="utf-8",
        )
        ...
        env = os.environ.copy()
        env.pop("TEMPO_FECHADO_MODO_SERVIDOR", None)
        env.pop("TEMPO_FECHADO_PONTO_PDFS_DIR", None)
        env["TEMPO_FECHADO_HOST"] = "127.0.0.1"
        env["TEMPO_FECHADO_PORT"] = str(porta)
        subprocess.Popen([str(exe)] + args, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, stdin=subprocess.DEVNULL, env=env)

    url_local = f"http://127.0.0.1:{porta}"
    registrar_historico_operacao_v8190(
        "Servidor Assistido",
        "OK",
        "Reinicio em modo local agendado pela interface.",
        {"exe": str(exe), "porta": porta, "url_local": url_local, "helper": str(helper) if helper else ""},
        origem="Servidor Assistido",
    )
    return {
        "ok": True,
        "erro": "",
        "mensagem": "O Tempo Fechado vai fechar o modo Servidor e reabrir em modo Local.",
        "ja_ativo": False,
        "url_local": url_local,
        "teste_local_url": url_local.rstrip("/") + "/saude-servidor",
    }
...
def _info_servidor_assistido_v82142():
    base = _ponto_pdfs_base_v82139()
    host = os.environ.get("TEMPO_FECHADO_HOST") or ("0.0.0.0" if _modo_servidor_ativo_v82139() else "127.0.0.1")
    try:
        porta = int(os.environ.get("TEMPO_FECHADO_PORT", "5050") or 5050)
    except Exception:
        porta = 5050
    ...
    for nome in ["entrada", "saida", "processados", "logs", "backup", "erro", "config", "controle", "backup_atualizacao"]:
        ...
        pastas.append({
            "nome": nome,
            "caminho": str(caminho),
            "existe": caminho.exists() and caminho.is_dir(),
        })
    arquivos = {
        "trava": str(ARQUIVO_TRAVA_PROCESSAMENTO_V82141),
        "historico_fila": str(ARQUIVO_HISTORICO_FILA_V82141),
        "historico_operacao": str(ARQUIVO_HISTORICO_OPERACAO_V8190),
    }
    trava = _trava_operacional_atual_v82140()
    checklist_servidor = _checklist_pacote_servidor_v82145(base, host, porta, pastas)
    acesso_usuarios = _acesso_usuarios_servidor_v82146(
        base=base,
        host=host,
        porta=porta,
        ip_rede=ip_rede,
        url_local=url_local,
        url_rede=url_rede,
        checklist=checklist_servidor,
        estrutura_ok=all(p["existe"] for p in pastas),
    )
    checklist_producao = _checklist_producao_servidor_v82159(
        modo_servidor=_modo_servidor_ativo_v82139(),
        host=host,
        porta=porta,
        estrutura_ok=all(p["existe"] for p in pastas),
        pacote_ok=bool(checklist_servidor.get("ok")),
        acesso_ok=bool(acesso_usuarios.get("ok")),
        url_rede=url_rede,
        teste_acesso_url=acesso_usuarios.get("teste_acesso_url", ""),
        trava=trava,
    )
    return {
        "ok": True,
        ...
        "url_local": url_local,
        "url_rede": url_rede,
        "url_requisicao": request.host_url.rstrip("/") if has_request_context() else url_local,
        "ponto_pdfs_base": str(base),
        "app_base_dir": APP_BASE_DIR,
        "iniciado_em": APP_STARTED_AT,
        "trava_operacional": trava,
        "historico_fila": _ler_historico_fila_v82141(12),
        "pastas": pastas,
        "arquivos": arquivos,
        "pacote_servidor": checklist_servidor,
        "acesso_usuarios": acesso_usuarios,
        "checklist_producao": checklist_producao,
        "estrutura_ok": all(p["existe"] for p in pastas),
    }


def _checklist_producao_servidor_v82159(modo_servidor, host, porta, estrutura_ok, pacote_ok, acesso_ok, url_rede, teste_acesso_url, trava):
    trava = trava or {}
    host_rede = str(host or "") in {"0.0.0.0", "::"} or not str(host or "").startswith("127.")
    itens = [
        {
            "nome": "Modo servidor",
            "ok": bool(modo_servidor),
            "status": "Servidor ativo" if modo_servidor else "Modo local",
            "acao": "Clicar em Reiniciar agora como Servidor.",
        },
        {
            "nome": "Host de rede",
            "ok": bool(host_rede),
            "status": str(host or "-"),
            "acao": "O servidor deve usar 0.0.0.0 para aceitar acesso remoto.",
        },
        {
            "nome": "Porta",
            "ok": int(porta or 0) > 0,
            "status": str(porta or "-"),
            "acao": "Manter a porta liberada no firewall.",
        },
        {
            "nome": "Pastas centrais",
            "ok": bool(estrutura_ok),
            "status": "Estrutura OK" if estrutura_ok else "Validar estrutura",
            "acao": "Clicar em Validar pastas.",
        },
        {
            "nome": "Endereco de rede",
            "ok": bool(url_rede and not str(url_rede).startswith("http://127.")),
            "status": str(url_rede or "-"),
            "acao": "Copiar este endereco para os usuarios.",
        },
        {
            "nome": "Teste remoto",
            "ok": bool(acesso_ok and teste_acesso_url),
            "status": str(teste_acesso_url or "-"),
            "acao": "Abrir este teste em outro notebook.",
        },
        {
            "nome": "Fila operacional",
            "ok": not bool(trava.get("ativa")),
            "status": "Livre" if not trava.get("ativa") else "Ocupada",
            "acao": "Aguardar o processamento terminar antes de nova execucao.",
        },
    ]
    pronto = all(bool(i.get("ok")) for i in itens)
    return {
        "ok": pronto and bool(pacote_ok),
        "status": "Pronto para producao" if pronto and pacote_ok else "Revisar antes de liberar",
        "itens": itens,
        "resumo": "Servidor pronto para receber usuarios da rede." if pronto and pacote_ok else "Revise os itens em atencao antes de distribuir o endereco.",
    }


def _checklist_pacote_servidor_v82145(base, host, porta, pastas):
    app_dir = Path(APP_BASE_DIR)
    candidatos_bat = [
        app_dir / "_internal" / "iniciar_tempo_fechado_servidor.bat",
        app_dir / "iniciar_tempo_fechado_servidor.bat",
        app_dir / "_internal" / "iniciar_modo_servidor_central.bat",
        app_dir / "iniciar_modo_servidor_central.bat",
    ]
    bat_oficial = next((p for p in candidatos_bat if p.exists()), candidatos_bat[0])
    checks = []

    def add(nome, ok, detalhe, acao=""):
        checks.append({"nome": nome, "ok": bool(ok), "detalhe": str(detalhe or ""), "acao": str(acao or "")})

    base = Path(base)
    add("Modo servidor", _modo_servidor_ativo_v82139(), "Ativo" if _modo_servidor_ativo_v82139() else "Local", "Clicar em Reiniciar agora como Servidor na guia Servidor.")
    add("Host de rede", str(host) in {"0.0.0.0", "::"} or not str(host).startswith("127."), str(host), "Usar host 0.0.0.0 para acesso na rede.")
    add("Porta", int(porta or 0) > 0, str(porta), "Manter uma porta fixa para os usuarios.")
    add("Pasta central", base.exists() and base.is_dir(), str(base), "Criar ou ajustar TEMPO_FECHADO_PONTO_PDFS_DIR.")
    add("Escrita na pasta central", _testar_escrita_pasta_v82145(base), str(base), "Conceder permissao de escrita ao usuario do servidor.")
    pendentes = [p.get("nome") for p in pastas or [] if not p.get("existe")]
    add("Estrutura ponto_pdfs", not pendentes, "OK" if not pendentes else "Pendentes: " + ", ".join(pendentes), "Validar pastas na guia Servidor.")
    add("Acionamento servidor", True, "Disponivel pela guia Servidor", "Usar o botao Reiniciar agora como Servidor.")
    add("URL de rede", bool(_ip_rede_servidor_v82142()), f"http://{_ip_rede_servidor_v82142()}:{porta}", "Compartilhar este endereco com os usuarios.")
    return {
        "ok": all(c["ok"] for c in checks),
        "checks": checks,
        "bat_oficial": str(bat_oficial),
        "comando_sugerido": f'TempoFechado.exe --server --data-dir "{base}" --host 0.0.0.0 --port {porta}',
        "variaveis": {
            "TEMPO_FECHADO_MODO_SERVIDOR": "1",
            "TEMPO_FECHADO_HOST": "0.0.0.0",
            "TEMPO_FECHADO_PORT": str(porta),
            "TEMPO_FECHADO_PONTO_PDFS_DIR": str(base),
        },
    }


def _testar_escrita_pasta_v82145(pasta):
    try:
        pasta = Path(pasta)
        pasta.mkdir(parents=True, exist_ok=True)
        teste = pasta / ".tempo_fechado_teste_escrita.tmp"
...
def _acesso_usuarios_servidor_v82146(base, host, porta, ip_rede, url_local, url_rede, checklist, estrutura_ok):
    modo_servidor = _modo_servidor_ativo_v82139()
    endereco = url_rede if modo_servidor and ip_rede else url_local
    teste_acesso_url = endereco.rstrip("/") + "/saude-servidor"
    pronto = bool(modo_servidor and estrutura_ok and (checklist or {}).get("ok") and endereco)
    status = "Servidor operacional" if pronto else "Aguardando modo servidor"
    convite = "\n".join([
        "Acesso ao Tempo Fechado",
        "",
        f"Endereco: {endereco}",
        f"Teste de acesso: {teste_acesso_url}",
        "Use seu usuario e senha cadastrados no sistema.",
        "Em caso de indisponibilidade, confirme se o computador servidor esta ligado e com o Tempo Fechado em modo servidor.",
    ])
    passos = [
        "Instalar a v8.21.67 no computador servidor.",
        "Abrir a guia Servidor e clicar em Reiniciar agora como Servidor.",
        "Validar o checklist da guia Servidor.",
        "Copiar o endereco de rede e enviar aos usuarios autorizados.",
        "Manter os PDFs concentrados pelo upload manual da plataforma.",
    ]
    kit_operacional = "\n".join([
        "Tempo Fechado - Acesso Operacional",
        "",
        f"Versao: {APP_FULL_NAME}",
        f"Status: {status}",
        f"Endereco do sistema: {endereco}",
        f"Teste de acesso: {teste_acesso_url}",
        f"Pasta central: {base}",
        "",
        "Sequencia recomendada:",
        "1. Abrir a guia Servidor e clicar em Reiniciar agora como Servidor.",
        "2. Abrir o link de teste em outro notebook.",
        "3. Confirmar login com usuario autorizado.",
        "4. Enviar PDFs pela guia Processamento.",
        "5. Acompanhar fila e governanca pelas guias Servidor e Governança.",
    ])
    return {
        "ok": pronto,
        "status": status,
        "selo": "Servidor operacional" if pronto else "Aguardando modo servidor",
        "endereco_recomendado": endereco,
        "teste_acesso_url": teste_acesso_url,
        "endereco_local": url_local,
        "endereco_rede": url_rede,
        "host": str(host),
        "porta": int(porta or 0),
        "pasta_central": str(base),
        "convite": convite,
        "kit_operacional": kit_operacional,
        "passos": passos,
        "observacao": "O acesso externo depende da rede local liberar o computador servidor e a porta configurada.",
    }


def _saude_publica_servidor_v82147():
    info = _info_servidor_assistido_v82142()
    acesso = info.get("acesso_usuarios") or {}
    trava = info.get("trava_operacional") or {}
    pacote = info.get("pacote_servidor") or {}
    return {
        "ok": True,
        "versao": APP_VERSION,
        "release": APP_RELEASE_NAME,
        "status": "Disponivel" if acesso.get("ok") else "Disponivel com pendencias",
        "modo": "Servidor Central" if info.get("modo_servidor") else "Local",
        "endereco": acesso.get("endereco_recomendado") or info.get("url_requisicao") or info.get("url_local"),
        "teste_acesso_url": acesso.get("teste_acesso_url", ""),
        "login_url": acesso.get("endereco_recomendado") or info.get("url_requisicao") or info.get("url_local"),
        "estrutura_ok": bool(info.get("estrutura_ok")),
        "pacote_ok": bool(pacote.get("ok")),
        "fila": "Ocupada" if trava.get("ativa") else "Livre",
        "mensagem": trava.get("mensagem") or "Servidor respondendo.",
        "iniciado_em": info.get("iniciado_em", ""),
        "consultado_em": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    }


def _html_saude_publica_servidor_v82147(dados):
    status_classe = "ok" if dados.get("estrutura_ok") and dados.get("pacote_ok") else "warn"
    linhas = [
        ("Versao", dados.get("versao", "")),
        ("Release", dados.get("release", "")),
        ("Modo", dados.get("modo", "")),
        ("Endereco", dados.get("endereco", "")),
        ("Estrutura", "OK" if dados.get("estrutura_ok") else "Revisar"),
        ("Pacote servidor", "OK" if dados.get("pacote_ok") else "Revisar"),
        ("Fila", dados.get("fila", "")),
        ("Inicio", dados.get("iniciado_em", "")),
        ("Consulta", dados.get("consultado_em", "")),
    ]
    cards = "\n".join(
        f"<div><strong>{html.escape(str(k))}</strong><span>{html.escape(str(v or '-'))}</span></div>"
        for k, v in linhas
    )
    mensagem = html.escape(str(dados.get("mensagem") or "Servidor respondendo."))
    login_url = html.escape(str(dados.get("login_url") or dados.get("endereco") or "#"), quote=True)
    return f"""<!doctype html>
<html lang="pt-br">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Tempo Fechado - Teste de Acesso</title>
  <style>
    body {{ margin: 0; font-family: Arial, sans-serif; background: #f4f7fb; color: #10213f; }}
    main {{ max-width: 880px; margin: 40px auto; padding: 0 18px; }}
    .hero {{ background: #fff; border: 1px solid #d8e2f0; border-radius: 8px; padding: 24px; }}
    .badge {{ display: inline-block; font-size: 12px; font-weight: 700; letter-spacing: .04em; text-transform: uppercase; color: #fff; background: #12305a; padding: 7px 10px; border-radius: 6px; }}
    h1 {{ margin: 16px 0 8px; font-size: 28px; }}
    p {{ line-height: 1.5; color: #354a68; }}
    .status {{ margin-top: 18px; padding: 14px; border-radius: 8px; border: 1px solid #cbd8e8; background: #eef7f0; }}
    .status.warn {{ background: #fff7e6; }}
    .grid {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(210px, 1fr)); gap: 10px; margin-top: 18px; }}
    .grid div {{ background: #fff; border: 1px solid #d8e2f0; border-radius: 8px; padding: 14px; }}
    .actions {{ margin-top: 18px; display: flex; gap: 10px; flex-wrap: wrap; }}
    a {{ display: inline-block; text-decoration: none; color: #fff; background: #12305a; padding: 11px 14px; border-radius: 6px; font-weight: 700; }}
    a.secondary {{ background: #eef3f8; color: #12305a; border: 1px solid #cbd8e8; }}
    strong {{ display: block; font-size: 12px; text-transform: uppercase; color: #526780; margin-bottom: 6px; }}
    span {{ font-size: 15px; word-break: break-word; }}
  </style>
</head>
<body>
  <main>
    <section class="hero">
      <span class="badge">Tempo Fechado</span>
      <h1>Teste de acesso ao servidor</h1>
      <p>Se esta pagina abriu no notebook do usuario, o servidor central esta respondendo na rede.</p>
      <div class="status {status_classe}"><strong>{html.escape(str(dados.get("status") or ""))}</strong><span>{mensagem}</span></div>
      <div class="grid">{cards}</div>
      <div class="actions">
        <a href="{login_url}">Entrar no Tempo Fechado</a>
        <a class="secondary" href="/api/saude-servidor">Ver JSON tecnico</a>
      </div>
    </section>
  </main>
</body>
...
def _preparar_env_script_embarcado_v82125(env, script):
    script = Path(script)
    env["TEMPO_FECHADO_PONTO_PDFS_DIR"] = str(_ponto_pdfs_base_v82139())
    if _modo_servidor_ativo_v82139():
        env["TEMPO_FECHADO_MODO_SERVIDOR"] = "1"
    ...
    try:
        ...
        if not launcher.exists():
            raise FileNotFoundError(
                "Componente de coleta não encontrado. Reinstale o Tempo Fechado ou confira a configuração da coleta."
            )

        timeout = int(opcoes.get("timeout_segundos") or 3600)
        _atualizar_job_processamento_integrado(
            fase="Executando coleta",
        ...
        _salvar_log_processamento_integrado("Execucao da coleta integrada - v8.21.67 Horas Extras Simplificadas", saida)
        ...
        if proc.returncode != 0 and _launcher_sem_pdfs_v82151(proc):
            mensagem_sem_pdfs = (
                "Coleta executada, mas nenhum PDF compativel foi encontrado no Outlook. "
                "Confira remetente, assunto, e-mails lidos/nao lidos, anexos e limite de mensagens analisadas."
            )
            resultado["launcher"].update({
                "ok": True,
                "sem_pdfs": True,
                "rotulo": "Sem PDFs",
                "mensagem": mensagem_sem_pdfs,
            })
            resultado["importacao_excel"] = {
                "ok": True,
                "executado": False,
                "mensagem": "Importacao nao executada porque nenhum PDF novo foi coletado.",
            }
            segundos = round(time.monotonic() - t0, 1)
            _atualizar_job_processamento_integrado(
                status="concluido", ok=True, fase="Coleta sem PDFs",
                mensagem=mensagem_sem_pdfs,
                finalizado_em=_agora_txt(), segundos=segundos, resultado=resultado,
            )
            registrar_auditoria("PROCESSAMENTO_INTEGRADO", mensagem_sem_pdfs, "AVISO")
            registrar_historico_operacao_v8190(
                "Processamento Integrado", "AVISO",
                mensagem_sem_pdfs, resultado, segundos=segundos, origem="Processamento"
            )
            return
        if proc.returncode != 0:
            detalhe = stderr_tail or stdout_tail or "Sem saída de erro capturada pelo launcher."
            raise RuntimeError(
                f"A coleta integrada terminou com erro. Código de retorno: {proc.returncode}. "
        ...
        try:
            ...
            status_trava = "erro" if job.get("status") == "erro" else "concluido"
            _liberar_trava_processamento_v82140(lock_id, status_trava, job.get("mensagem") or "Processamento integrado finalizado.")
        except Exception:
            pass
...
def _registrar_prova_chamada_launcher_v81725(script_path="", comando=""):
    try:
        controle = _ponto_pdfs_base_v82139() / "controle"
...
def api_diagnostico_pos_instalacao_v82128():
    ...
    itens.append(_diag_item_v82128("Aplicação", True, "Serviço principal carregado.", ""))
    itens.append(_diag_item_v82128("Coleta integrada", launcher.exists(), "Componente disponível." if launcher.exists() else "Componente indisponível.", "Regerar instalador se a coleta não estiver disponível."))
    itens.append(_diag_item_v82128("Processamento manual", script_robo.exists(), "Componente disponível." if script_robo.exists() else "Componente indisponível.", "Regerar instalador se o processamento manual não estiver disponível."))
...
@login_obrigatorio
@permissao_operacional_v82143("upload_pdfs", "upload de PDFs")
...
@login_obrigatorio
@permissao_operacional_v82143("processar", "processamento manual")
def api_processamento_manual_robo_executar_v81737():
    ...
    ok_trava, trava = _adquirir_trava_processamento_v82140(
        ...
        "Processamento Manual",
        f"Robo manual em andamento com {len(pdfs)} PDF(s).",
    )
    if not ok_trava:
        return _resposta_trava_ocupada_v82140(trava)
    status_trava = "concluido"
    mensagem_trava = "Processamento manual finalizado."
    try:
...
@login_obrigatorio
@permissao_operacional_v82143("processar", "importacao manual do Excel")
...
def _montar_prevalidacao_processamento_v82117():
    ...
    add("Coleta integrada", launcher.exists(), "Disponível" if launcher.exists() else "Indisponível")
...
@login_obrigatorio
@permissao_operacional_v82143("processar", "acompanhamento do processamento")
def api_processamento_integrado_status():
    return jsonify(_job_processamento_integrado_atual())


@app.route("/api/processamento/trava", methods=["GET"])
@login_obrigatorio
@admin_obrigatorio
def api_processamento_trava_v82140():
    return jsonify({
        "ok": True,
        "erro": "",
        "trava_operacional": _trava_operacional_atual_v82140(),
        "historico_fila": _ler_historico_fila_v82141(20),
        "arquivo_trava": str(ARQUIVO_TRAVA_PROCESSAMENTO_V82141),
        "arquivo_historico_fila": str(ARQUIVO_HISTORICO_FILA_V82141),
    })


@app.route("/api/processamento/trava/liberar", methods=["POST"])
@login_obrigatorio
@admin_obrigatorio
def api_processamento_trava_liberar_v82140():
    trava = _liberar_trava_processamento_v82140(None, "liberado", "Trava operacional liberada manualmente pelo administrador.")
    registrar_historico_operacao_v8190(
        "Trava Operacional",
        "OK",
        "Trava operacional liberada manualmente pelo administrador.",
        {"trava": trava},
        origem="Processamento"
    )
    return jsonify({"ok": True, "erro": "", "mensagem": "Trava operacional liberada.", "trava_operacional": trava})


@app.route("/api/servidor-assistido", methods=["GET"])
@login_obrigatorio
@permissao_operacional_v82143("processar", "acompanhamento do servidor")
def api_servidor_assistido_v82142():
    return jsonify(_info_servidor_assistido_v82142())


@app.route("/api/saude-servidor", methods=["GET"])
def api_saude_servidor_v82147():
    return jsonify(_saude_publica_servidor_v82147())


@app.route("/saude-servidor", methods=["GET"])
def pagina_saude_servidor_v82147():
    return _html_saude_publica_servidor_v82147(_saude_publica_servidor_v82147())


@app.route("/api/servidor-assistido/validar-estrutura", methods=["POST"])
@login_obrigatorio
@admin_obrigatorio
def api_servidor_assistido_validar_estrutura_v82142():
    base = garantir_estrutura_ponto_pdfs_v81720()
    info = _info_servidor_assistido_v82142()
    registrar_historico_operacao_v8190(
        "Servidor Assistido",
        "OK" if info.get("estrutura_ok") else "ATENCAO",
        "Estrutura de pastas do servidor validada.",
        {"base": base, "pastas": info.get("pastas", [])},
        origem="Servidor Assistido",
    )
    info["mensagem"] = "Estrutura de pastas validada."
    return jsonify(info)


@app.route("/api/servidor-assistido/ativar", methods=["POST"])
@login_obrigatorio
@permissao_operacional_v82143("processar", "ativacao do modo servidor")
def api_servidor_assistido_ativar_v82154():
    try:
        return jsonify(_agendar_reinicio_modo_servidor_v82154())
    except Exception as e:
        LOGGER.exception(f"Falha ao ativar modo servidor pela interface: {e}")
        return jsonify({"ok": False, "erro": f"Falha ao ativar modo servidor: {e}"}), 500


@app.route("/api/servidor-assistido/reiniciar-servidor", methods=["POST"])
@login_obrigatorio
@permissao_operacional_v82143("processar", "reinicio em modo servidor")
def api_servidor_assistido_reiniciar_servidor_v82156():
    try:
        return jsonify(_agendar_reinicio_modo_servidor_v82154())
    except Exception as e:
        LOGGER.exception(f"Falha ao reiniciar em modo servidor pela interface: {e}")
        return jsonify({"ok": False, "erro": f"Falha ao reiniciar como servidor: {e}"}), 500


@app.route("/api/servidor-assistido/reiniciar-local", methods=["POST"])
@login_obrigatorio
@permissao_operacional_v82143("processar", "retorno ao modo local")
def api_servidor_assistido_reiniciar_local_v82165():
    try:
        return jsonify(_agendar_reinicio_modo_local_v82165())
    except Exception as e:
        LOGGER.exception(f"Falha ao reiniciar em modo local pela interface: {e}")
        return jsonify({"ok": False, "erro": f"Falha ao voltar ao modo local: {e}"}), 500


@app.route("/api/servidor-assistido/abrir-pasta-central", methods=["POST"])
@login_obrigatorio
@admin_obrigatorio
def api_servidor_assistido_abrir_pasta_central_v82142():
    base = _ponto_pdfs_base_v82139()
    try:
        ...
        if sys.platform.startswith("win"):
            ...
            subprocess.Popen(["xdg-open", caminho], shell=False)
        return jsonify({"ok": True, "erro": "", "mensagem": "Pasta central aberta.", "ponto_pdfs_base": caminho})
    except Exception as e:
        return jsonify({"ok": False, "erro": f"Falha ao abrir pasta central: {e}", "ponto_pdfs_base": str(base)}), 500


@app.route("/api/ciclo-operacional", methods=["GET"])
@login_obrigatorio
@permissao_operacional_v82143("processar", "acompanhamento do ciclo operacional")
...
@login_obrigatorio
@permissao_operacional_v82143("processar", "pre-validacao do processamento")
...
@login_obrigatorio
@permissao_operacional_v82143("processar", "historico de processamentos")
def api_processamento_historico_v82117():
    try:
        limite = int(request.args.get("limite", 20) or 20)
    except Exception:
        limite = 20
    return jsonify({"ok": True, "erro": "", "historico": _ler_historico_operacao_v8190(limite), "historico_fila": _ler_historico_fila_v82141(limite)})


@app.route("/api/processamento-integrado/iniciar", methods=["POST"])
@login_obrigatorio
@permissao_operacional_v82143("processar", "processamento integrado")
def api_processamento_integrado_iniciar():
    ...
    ok_trava, trava = _adquirir_trava_processamento_v82140(
        ...
        "Processamento",
        "Processamento integrado iniciado em segundo plano.",
    )
    if not ok_trava:
        return _resposta_trava_ocupada_v82140(trava)
    with PROCESSAMENTO_INTEGRADO_LOCK:
    ...
    opcoes = {
        ...
        "lock_id": trava.get("id"),
    }
    try:
        th = threading.Thread(target=_executar_processamento_integrado_worker, args=(opcoes,), daemon=True)
        th.start()
    except Exception as e:
        _liberar_trava_processamento_v82140(trava.get("id"), "erro", f"Falha ao iniciar thread: {e}")
        raise
...
def api_processamento_nova_rodada_v82117():
    ...
    ok_trava, trava = _adquirir_trava_processamento_v82140(
        "Nova Rodada",
        "Processamento",
        "Nova rodada operacional em andamento.",
    )
    if not ok_trava:
        return _resposta_trava_ocupada_v82140(trava)
    ...
    try:
        if bool(payload.get("executar_cleanup", True)):
            ...
            if status_code >= 400 or not cleanup_resultado.get("ok"):
                _liberar_trava_processamento_v82140(trava.get("id"), "erro", cleanup_resultado.get("erro") or "Cleanup bloqueou a nova rodada.")
                return jsonify(cleanup_resultado), status_code
    except Exception as e:
        _liberar_trava_processamento_v82140(trava.get("id"), "erro", f"Falha no cleanup da nova rodada: {e}")
        raise
    ...
    if request.method == "GET":
        ...
        payload = {
            ...
            "outlook": {
                ...
                "observacao": "A política de captura e o limite de e-mails são aplicados na próxima execução da coleta.",
            },
...
def _ler_auditoria_multiusuario_v82144(limite=200):
    linhas = []
    try:
        if not ARQUIVO_AUDITORIA_MULTIUSUARIO.exists():
            return []
        raw = ARQUIVO_AUDITORIA_MULTIUSUARIO.read_text(encoding="utf-8", errors="ignore").splitlines()
        for linha in raw[-max(int(limite or 200), 1):]:
            partes = linha.split("\t")
            while len(partes) < 8:
                partes.append("")
            linhas.append({
...
def _governanca_contagem_por_campo_v82144(itens, campo):
    contagem = {}
    for item in itens or []:
        chave = str((item or {}).get(campo) or "").strip() or "-"
        contagem[chave] = contagem.get(chave, 0) + 1
    return [{"nome": k, "qtd": v} for k, v in sorted(contagem.items(), key=lambda x: (-x[1], x[0].lower()))]


def _montar_governanca_servidor_v82144():
    eventos = _ler_historico_operacao_v8190(30)
    auditoria = _ler_auditoria_multiusuario_v82144(200)
    fila = _ler_historico_fila_v82141(30)
    saude = _status_saude_operacional_v8190(eventos)
    trava = _trava_operacional_atual_v82140()
    base = _ponto_pdfs_base_v82139()
    pastas = {
        "entrada": _contar_pdfs_pasta_v8190(base / "entrada"),
        "saida": _contar_pdfs_pasta_v8190(base / "saida"),
        "processados": _contar_pdfs_pasta_v8190(base / "processados"),
        "erro": _contar_pdfs_pasta_v8190(base / "erro"),
    }
    usuarios = listar_usuarios(ARQUIVO_USUARIOS_DB)
    perfis = {}
    ativos = 0
    for usuario in usuarios:
        perfil = str(usuario.get("perfil") or "consulta")
        perfis[perfil] = perfis.get(perfil, 0) + 1
        if usuario.get("ativo"):
            ativos += 1

    uploads = [a for a in auditoria if "UPLOAD" in str(a.get("acao", "")).upper() or "PDF" in str(a.get("detalhe", "")).upper()]
    logins = [a for a in auditoria if str(a.get("acao", "")).upper() in {"LOGIN", "LOGIN_API"}]
    bloqueios = [e for e in fila if str(e.get("evento", "")).lower() == "bloqueio"]
    erros = [e for e in eventos if str(e.get("status", "")).upper() in {"ERRO", "FALHA"}]

    riscos = []

    def add_risco(nivel, titulo, detalhe, acao=""):
        riscos.append({"nivel": nivel, "titulo": titulo, "detalhe": detalhe, "acao": acao})

    if pastas["entrada"] >= 20:
        add_risco("alto", "Fila de entrada elevada", f"{pastas['entrada']} PDF(s) aguardando processamento.", "Priorizar uma rodada de processamento.")
    elif pastas["entrada"] >= 5:
        add_risco("medio", "Fila de entrada em crescimento", f"{pastas['entrada']} PDF(s) aguardando processamento.", "Acompanhar a fila nesta rodada.")
    if pastas["erro"] > 0:
        add_risco("medio", "PDFs em erro", f"{pastas['erro']} PDF(s) na pasta de erro.", "Revisar arquivos rejeitados.")
    if trava.get("ativa"):
        nivel = "alto" if trava.get("recuperada_apos_reinicio") else "medio"
        add_risco(nivel, "Trava operacional ativa", trava.get("mensagem") or "Processamento em andamento.", "Acompanhar conclusao antes de iniciar nova execucao.")
    if len(erros) >= 3:
        add_risco("alto", "Erros recentes recorrentes", f"{len(erros)} erro(s) nos ultimos eventos operacionais.", "Abrir Central de Operacao e revisar log.")
    elif erros:
        add_risco("medio", "Erro operacional recente", erros[0].get("mensagem") or erros[0].get("resumo") or "Evento com erro registrado.", "Revisar ultimo evento.")
    if bloqueios:
        add_risco("baixo", "Bloqueios de fila registrados", f"{len(bloqueios)} tentativa(s) bloqueada(s) pela trava operacional.", "Validar se a fila esta sendo usada corretamente.")
    ...
    return {
        ...
        "resumo": {
            ...
            "perfis": perfis,
            "pdfs_entrada": pastas["entrada"],
            "pdfs_saida": pastas["saida"],
            "pdfs_processados": pastas["processados"],
            "pdfs_erro": pastas["erro"],
            "eventos_erro": len(erros),
            "bloqueios_fila": len(bloqueios),
            "trava_ativa": bool(trava.get("ativa")),
            "score_saude": saude.get("score", 0),
        },
        "riscos": riscos[:8],
        "trava_operacional": trava,
        "saude": saude,
        "atividade_por_usuario": _governanca_contagem_por_campo_v82144(auditoria, "usuario")[:10],
        "atividade_por_acao": _governanca_contagem_por_campo_v82144(auditoria, "acao")[:10],
        "ultimos_logins": logins[:8],
        "ultimos_uploads": uploads[:8],
        "eventos": eventos[:12],
        "historico_fila": fila[:12],
        "auditoria": auditoria[:20],
    }


@app.route("/api/governanca-servidor", methods=["GET"])
@login_obrigatorio
@permissao_operacional_v82143("processar", "governanca do servidor")
def api_governanca_servidor_v82144():
    return jsonify(_montar_governanca_servidor_v82144())
...
if __name__ == "__main__":
    host_exec = os.environ.get("TEMPO_FECHADO_HOST") or ("0.0.0.0" if _modo_servidor_ativo_v82139() else "127.0.0.1")
    try:
        porta_exec = int(os.environ.get("TEMPO_FECHADO_PORT", "5050") or 5050)
    except Exception:
        porta_exec = 5050
    app.run(debug=False, host=host_exec, port=porta_exec, threaded=True)

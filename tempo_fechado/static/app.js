...
// v8.21.67 - Horas Extras Simplificadas.
...
let barraRolagemHorizontalV82153 = null;
let barraRolagemHorizontalInternaV82153 = null;
let tabelaWrapAtivaV82153 = null;
let sincronizandoRolagemV82153 = false;
const wrapsComScrollMonitoradoV82153 = new WeakSet();

function elementoVisivelV82153(el) {
  if (!el) return false;
...
function obterTabelaWrapAtivaV82153() {
  const wraps = Array.from(document.querySelectorAll(".table-wrap"));
  return wraps.find(wrap => elementoVisivelV82153(wrap)) || null;
}

function monitorarWrapRolagemV82153(wrap) {
  if (!wrap || wrapsComScrollMonitoradoV82153.has(wrap)) return;
  wrapsComScrollMonitoradoV82153.add(wrap);
  wrap.addEventListener("scroll", () => {
    if (sincronizandoRolagemV82153 || tabelaWrapAtivaV82153 !== wrap || !barraRolagemHorizontalV82153) return;
    sincronizandoRolagemV82153 = true;
    barraRolagemHorizontalV82153.scrollLeft = wrap.scrollLeft;
    sincronizandoRolagemV82153 = false;
  }, { passive: true });
}

function posicionarBarraRolagemHorizontalV82153() {
  if (!barraRolagemHorizontalV82153) return;
  const header = document.querySelector(".table-card .table-header");
  if (header && barraRolagemHorizontalV82153.previousElementSibling !== header) {
    header.insertAdjacentElement("afterend", barraRolagemHorizontalV82153);
  }
}

function atualizarBarraRolagemHorizontalV82153() {
  if (!barraRolagemHorizontalV82153 || !barraRolagemHorizontalInternaV82153) return;
  posicionarBarraRolagemHorizontalV82153();
  const wrap = obterTabelaWrapAtivaV82153();
  tabelaWrapAtivaV82153 = wrap;
  if (!wrap) {
    barraRolagemHorizontalV82153.classList.add("hidden");
    return;
  }
  monitorarWrapRolagemV82153(wrap);
  const largura = Math.max(wrap.scrollWidth, wrap.clientWidth + 1);
  barraRolagemHorizontalInternaV82153.style.width = `${largura}px`;
  barraRolagemHorizontalV82153.classList.remove("hidden");
  if (!sincronizandoRolagemV82153) {
    barraRolagemHorizontalV82153.scrollLeft = wrap.scrollLeft;
  }
}

function inicializarBarraRolagemHorizontalV82153() {
  if (barraRolagemHorizontalV82153) {
    atualizarBarraRolagemHorizontalV82153();
    return;
  }
  barraRolagemHorizontalV82153 = document.createElement("div");
  barraRolagemHorizontalV82153.id = "barraRolagemHorizontalV82153";
  barraRolagemHorizontalV82153.className = "table-top-hscroll hidden";
  barraRolagemHorizontalV82153.setAttribute("aria-label", "Rolagem horizontal da tabela atual");
  barraRolagemHorizontalInternaV82153 = document.createElement("div");
  barraRolagemHorizontalInternaV82153.className = "table-top-hscroll-inner";
  barraRolagemHorizontalV82153.appendChild(barraRolagemHorizontalInternaV82153);
  posicionarBarraRolagemHorizontalV82153();

  barraRolagemHorizontalV82153.addEventListener("scroll", () => {
    if (sincronizandoRolagemV82153 || !tabelaWrapAtivaV82153) return;
    sincronizandoRolagemV82153 = true;
    tabelaWrapAtivaV82153.scrollLeft = barraRolagemHorizontalV82153.scrollLeft;
    sincronizandoRolagemV82153 = false;
  }, { passive: true });

  window.addEventListener("resize", () => requestAnimationFrame(atualizarBarraRolagemHorizontalV82153));
  document.addEventListener("scroll", () => requestAnimationFrame(atualizarBarraRolagemHorizontalV82153), true);
  if (window.ResizeObserver) {
    const ro = new ResizeObserver(() => requestAnimationFrame(atualizarBarraRolagemHorizontalV82153));
    ro.observe(document.body);
  }
  if (window.MutationObserver) {
    const mo = new MutationObserver(() => requestAnimationFrame(atualizarBarraRolagemHorizontalV82153));
    mo.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ["class", "style"] });
  }
  setInterval(atualizarBarraRolagemHorizontalV82153, 1200);
  atualizarBarraRolagemHorizontalV82153();
}
...
function limparSubtitulosGuiasV82130() {
  ...
  alvo.setAttribute("data-subtitulos-removidos", "v8.21.67");
}

document.addEventListener("DOMContentLoaded", () => {
  limparSubtitulosGuiasV82130();
  inicializarBarraRolagemHorizontalV82153();
  const alvo = qs("subtituloPagina");
...
let anotacoesOperacionaisCache = {};
let anotacoesOperacionaisPromise = null;
const anotacoesOperacionaisTimers = new Map();
const PAGINAS_EXPORTAVEIS_V82160 = new Set([
  "consolidado",
  "inconsistencias",
  "ponto_aberto",
  "ponto_aberto_impar",
...
const TITULOS_PAINEL_COMPARTILHADO_V81744 = {
  anotacoes_operacionais: "Anotações Operacionais",
  ...
  servidor_assistido: "Servidor",
  governanca_servidor: "Governança",
  central_operacao: "Central de Operação",
  diagnostico_instalacao: "Saúde do Sistema",
...
function chaveAnotacaoOperacional(guia, r) {
  const partes = [
    guia,
    r?.cc,
    r?.nome,
    r?.data || r?.data_referencia || r?.data_retorno || r?.data_anterior || "",
    r?.dia || "",
    r?.tipo_jornada || "",
    r?.violacao || "",
    r?.periodo || "",
  ...
  return partes.map(v => String(v ?? "").trim().toLowerCase()).join("|").slice(0, 260);
}
...
async function carregarAnotacoesOperacionais(force = false) {
  ...
  anotacoesOperacionaisPromise = fetch(`/tempo-fechado/api/anotacoes-operacionais?_t=${ts}`)
    ...
    .then(json => {
      ...
      return anotacoesOperacionaisCache;
    })
    .catch(e => {
      console.warn("Falha ao carregar anotacoes operacionais.", e);
      return anotacoesOperacionaisCache || {};
    });
...
function cabecalhoAnotacaoOperacional() {
  return `
    <th>Nome do Gestor</th>
    <th>Adm Respons&aacute;vel</th>
    <th>Observa&ccedil;&atilde;o</th>
  `;
}

function salvarAnotacaoOperacional(guia, chaveCodificada, campo, valor) {
  const chave = decodeURIComponent(chaveCodificada || "");
  if (!chave) return;
  const atual = Object.assign({}, anotacoesOperacionaisCache[chave] || {});
  atual[campo] = valor || "";
  atual.guia = guia;
  anotacoesOperacionaisCache[chave] = atual;

  if (anotacoesOperacionaisTimers.has(chave)) clearTimeout(anotacoesOperacionaisTimers.get(chave));
  const timer = setTimeout(async () => {
    ...
    try {
      ...
      console.warn("Falha ao salvar anotacao operacional.", e);
    } finally {
      anotacoesOperacionaisTimers.delete(chave);
    }
  }, 250);
  anotacoesOperacionaisTimers.set(chave, timer);
}
...
function atualizarIndicadoresOrdenacao() {
  ...
  if (saldoIcon) {
    saldoIcon.textContent =
      ordenacaoSaldo === "desc" ? " â†“" :
      ordenacaoSaldo === "asc" ? " â†‘" : " ↕";
  }
  ...
  if (incIcon) {
    incIcon.textContent =
      ordenacaoInconsistencias === "desc" ? " â†“" :
      ordenacaoInconsistencias === "asc" ? " â†‘" : " ↕";
  }
}
...
function atualizarVisibilidadeFiltroTipo() {
  ...
  const deveOcultar = paginaAtual === "hora_extra" || paginaAtual === "hora_extra_simplificada" || paginaAtual === "ausencias";
...
function setPaginaConsolidado() {
  ...
  } else if (paginaAtual === "hora_extra_simplificada") {
    renderizarCardsPorGuia();
    setCabecalhoHoraExtraSimplificadaV82167();
  ...
  qs("tituloTabela").textContent =
    paginaAtual === "inconsistencias"
      ...
      : paginaAtual === "hora_extra_simplificada"
        ? "Horas Extras Simplificadas"
        : "Base Consolidada";
...
function atualizarAcoesTopbarPorPagina() {
  ...
  if (btnExportar) {
    ...
    btnExportar.classList.toggle("hidden", ocultarExportar);
    btnExportar.style.display = ocultarExportar ? "none" : "";
    if (ocultarExportar) {
      btnExportar.setAttribute("aria-hidden", "true");
    } else {
      btnExportar.removeAttribute("aria-hidden");
    }
  }
}
...
function marcarPaginaAtualClasse() {
  ...
  document.body.classList.toggle("pagina-anotacoes-operacionais", paginaAtual === "anotacoes_operacionais");
  ...
  document.body.classList.toggle("pagina-servidor-assistido", paginaAtual === "servidor_assistido");
  document.body.classList.toggle("pagina-governanca-servidor", paginaAtual === "governanca_servidor");
...
function renderDatasSelecionadas() {
  ...
  box.innerHTML = datasFiltro.map(data => `
    <span class="date-chip">
      ${formatarDataBR(data)}
      <button type="button" onclick="removerDataFiltro('${data}')">Ã—</button>
    </span>
  `).join("");
}
...
function renderDatasDashboardSelecionadas() {
  ...
  box.innerHTML = datasDashboardFiltro.map(data => `
    <span class="date-chip">
      ${formatarDataBR(data)}
      <button type="button" onclick="removerDataDashboardFiltro('${data}')">Ã—</button>
    </span>
  `).join("");
}
...
async function carregarConsolidado() {
  ...
  if (["inconsistencias", "hora_extra", "hora_extra_simplificada"].includes(paginaAtual)) {
    await carregarAnotacoesOperacionais();
  }
  renderConsolidado();
...
async function carregarDashboard() {
  ...
  if (qs("tbodyDashboard")) {
    qs("tbodyDashboard").innerHTML = `<tr><td colspan="12">Consulta principal em execucao. A tabela sera preenchida assim que o resumo responder.</td></tr>`;
  }
  let avisoConsultaBancoHorasTimer = setTimeout(() => {
    ...
    if (qs("tbodyDashboard")) {
      qs("tbodyDashboard").innerHTML = `<tr><td colspan="12">Processo em execucao no servidor. Aguarde mais alguns instantes; filtros e painel auxiliar foram adiados para priorizar esta tabela.</td></tr>`;
    }
  }, 900);
  ...
  const renderLinhaDashboard = (r) => `
    <tr>
      ...
      ${renderCamposAnotacaoOperacional("banco_horas", r)}
    </tr>
  `;

  renderTabelaEmLotes("tbodyDashboard", dadosDashboardFiltrados, renderLinhaDashboard, {
    htmlVazio: `<tr><td colspan="12">${escapeHtml(json.aviso || "Nenhum colaborador carregado no resumo do Banco de Horas.")}</td></tr>`,
    aoConcluir: (total) => {
...
function usuarioPodeV82143(permissao) {
  const perms = (window.USUARIO_ATUAL && window.USUARIO_ATUAL.permissoes) || {};
...
function aplicarPermissoesInterface() {
  ...
  const podeSupervisor = usuarioPodeV82143("processar");
  document.querySelectorAll(".requires-supervisor").forEach(el => {
    el.classList.toggle("supervisor-hidden", !podeSupervisor);
    if (!podeSupervisor) el.setAttribute("aria-hidden", "true");
    else el.removeAttribute("aria-hidden");
  });
  const podeOperador = usuarioPodeV82143("upload_pdfs");
  document.querySelectorAll(".requires-operador").forEach(el => {
    el.classList.toggle("operador-hidden", !podeOperador);
    if (!podeOperador) el.setAttribute("aria-hidden", "true");
    else el.removeAttribute("aria-hidden");
  });
}
...
function renderCardsExtratoBancoHoras(resumo) {
  ...
  if (!temResumo) {
    container.innerHTML = `
      <div class="card"><span>Saldo Base</span><strong>â€”</strong></div>
      <div class="card"><span>Créditos</span><strong>â€”</strong></div>
      <div class="card"><span>Débitos</span><strong>â€”</strong></div>
      <div class="card"><span>Saldo Final</span><strong>â€”</strong></div>
      <div class="card"><span>Data Base</span><strong>â€”</strong></div>
      <div class="card"><span>Origem</span><strong>â€”</strong></div>
    `;
        ...
        <table class="diag-table diag-table-wide">
          <thead>
            <tr>
              ...
              <th>1? data</th><th>Última data</th><th>Saldo Anterior</th><th>Débito</th>
...
let anotacoesOperacionaisPainelListaV82161 = [];
let anotacoesOperacionaisPainelTimerV82161 = null;

const ROTULOS_GUIAS_ANOTACOES_V82161 = {
  banco_horas: "Banco de Horas",
  horas_extras: "Horas Extras",
  horas_extras_simplificadas: "Horas Extras Simplificadas",
  inconsistencias: "Inconsistências",
  sem_marcacao: "Sem Marcação",
  ponto_aberto: "Ponto Aberto",
  inter_jornada: "Inter Jornada",
  violacoes_jornada: "Violações de Jornada"
};

function rotuloGuiaAnotacaoV82161(guia) {
  return ROTULOS_GUIAS_ANOTACOES_V82161[guia] || String(guia || "Sem guia").replaceAll("_", " ");
}

function statusAnotacaoOperacionalV82161(item) {
  const gestor = String(item.nome_gestor || "").trim();
  const adm = String(item.adm_responsavel || "").trim();
  const obs = String(item.observacao || "").trim();
  if (gestor && adm && obs) return "Completa";
  if (obs && (!gestor || !adm)) return "Ação pendente";
  if (!obs && (gestor || adm)) return "Sem observação";
  return "Incompleta";
}

function montarListaAnotacoesOperacionaisV82161(anotacoes) {
  return Object.entries(anotacoes || {}).map(([chave, registro]) => {
    const partes = String(chave || "").split("|");
    const item = Object.assign({}, registro || {});
    item.chave = chave;
    item.guia = item.guia || partes[0] || "";
    item.cc = partes[1] || "";
    item.nome = partes[2] || "";
    item.data = partes[3] || "";
    item.dia = partes[4] || "";
    item.tipo_jornada = partes[5] || "";
    item.violacao = partes[6] || "";
    item.periodo = partes[7] || "";
    item.inconsistencias = partes[8] || "";
    item.saldo = partes[9] || "";
    item.status = statusAnotacaoOperacionalV82161(item);
    item.busca = [
      item.guia, item.cc, item.nome, item.data, item.dia, item.tipo_jornada,
      item.violacao, item.periodo, item.inconsistencias, item.saldo,
      item.nome_gestor, item.adm_responsavel, item.observacao, item.status
    ].join(" ").toLowerCase();
    return item;
  }).sort((a, b) => String(b.atualizado_em || "").localeCompare(String(a.atualizado_em || "")));
}

function filtrosPainelAnotacoesV82161() {
  const texto = String(qs("filtroAnotacoesTextoV82161")?.value || "").trim().toLowerCase();
  const guia = String(qs("filtroAnotacoesGuiaV82161")?.value || "");
  const gestor = String(qs("filtroAnotacoesGestorV82161")?.value || "").trim().toLowerCase();
  const status = String(qs("filtroAnotacoesStatusV82161")?.value || "");
  return { texto, guia, gestor, status };
}

function listaFiltradaAnotacoesV82161() {
  const f = filtrosPainelAnotacoesV82161();
  return anotacoesOperacionaisPainelListaV82161.filter(item => {
    if (f.texto && !item.busca.includes(f.texto)) return false;
    if (f.guia && item.guia !== f.guia) return false;
    if (f.gestor && !String(item.nome_gestor || "").toLowerCase().includes(f.gestor)) return false;
    if (f.status && item.status !== f.status) return false;
    return true;
  });
}

function opcoesGuiasAnotacoesV82161(lista) {
  const guias = Array.from(new Set((lista || []).map(i => i.guia).filter(Boolean))).sort((a, b) => rotuloGuiaAnotacaoV82161(a).localeCompare(rotuloGuiaAnotacaoV82161(b), "pt-BR"));
  return guias.map(guia => `<option value="${escapeHtml(guia)}">${escapeHtml(rotuloGuiaAnotacaoV82161(guia))}</option>`).join("");
}

function renderizarTabelaAnotacoesOperacionaisV82161() {
  const tbody = qs("tbodyAnotacoesOperacionaisV82161");
  const resumo = qs("resumoAnotacoesOperacionaisV82161");
  if (!tbody) return;

  const linhas = listaFiltradaAnotacoesV82161();
  const limite = 700;
  const renderizadas = linhas.slice(0, limite);
  tbody.innerHTML = renderizadas.map(item => `
    <tr>
      <td>${escapeHtml(rotuloGuiaAnotacaoV82161(item.guia))}</td>
      <td><strong>${escapeHtml(item.nome || "-")}</strong><small>${escapeHtml(item.cc || "")}</small></td>
      <td>${escapeHtml(item.data || item.periodo || "-")}</td>
      <td>${escapeHtml(item.violacao || item.inconsistencias || item.saldo || "-")}</td>
      <td>${escapeHtml(item.nome_gestor || "")}</td>
      <td>${escapeHtml(item.adm_responsavel || "")}</td>
      <td>${escapeHtml(item.observacao || "")}</td>
      <td><span class="badge ${item.status === "Completa" ? "success" : item.status === "Ação pendente" ? "warning" : "neutral"}">${escapeHtml(item.status)}</span></td>
      <td>${escapeHtml(item.atualizado_em || "")}<small>${escapeHtml(item.atualizado_por || "")}</small></td>
    </tr>
  `).join("");

  if (!renderizadas.length) {
    tbody.innerHTML = `<tr><td colspan="9" class="exec-empty">Nenhuma anotação encontrada para os filtros atuais.</td></tr>`;
  }

  if (resumo) {
    const corte = linhas.length > limite ? ` Exibindo ${limite} para preservar a fluidez.` : "";
    resumo.textContent = `${linhas.length} anotação(ões) encontrada(s).${corte}`;
  }
  if (qs("statusCarga") && paginaAtual === "anotacoes_operacionais") {
    qs("statusCarga").textContent = `${linhas.length} anotação(ões) no painel.`;
  }
  requestAnimationFrame(atualizarBarraRolagemHorizontalV82153);
}

function filtrarPainelAnotacoesOperacionaisV82161() {
  clearTimeout(anotacoesOperacionaisPainelTimerV82161);
  anotacoesOperacionaisPainelTimerV82161 = setTimeout(renderizarTabelaAnotacoesOperacionaisV82161, 120);
}

function limparFiltrosAnotacoesOperacionaisV82161() {
  ["filtroAnotacoesTextoV82161", "filtroAnotacoesGuiaV82161", "filtroAnotacoesGestorV82161", "filtroAnotacoesStatusV82161"].forEach(id => {
    if (qs(id)) qs(id).value = "";
  });
  renderizarTabelaAnotacoesOperacionaisV82161();
}

function renderizarPainelAnotacoesOperacionaisV82161(lista) {
  const painel = qs("dashboardExecutivoPremium");
  if (!painel) return;
  const total = lista.length;
  const completas = lista.filter(i => i.status === "Completa").length;
  const pendentes = lista.filter(i => i.status !== "Completa").length;
  const gestores = new Set(lista.map(i => String(i.nome_gestor || "").trim()).filter(Boolean)).size;

  painel.innerHTML = `
    <div class="op-notes-panel">
      <section class="op-notes-kpis">
        <div><span>Anotações</span><strong>${total}</strong></div>
        <div><span>Completas</span><strong>${completas}</strong></div>
        <div><span>Pendentes</span><strong>${pendentes}</strong></div>
        <div><span>Gestores citados</span><strong>${gestores}</strong></div>
      </section>
      <section class="filters op-notes-filters">
        <input id="filtroAnotacoesTextoV82161" type="search" placeholder="Buscar colaborador, CC, data, ocorrência ou observação" oninput="filtrarPainelAnotacoesOperacionaisV82161()">
        <select id="filtroAnotacoesGuiaV82161" onchange="filtrarPainelAnotacoesOperacionaisV82161()">
          <option value="">Todas as guias</option>
          ${opcoesGuiasAnotacoesV82161(lista)}
        </select>
        <input id="filtroAnotacoesGestorV82161" type="search" placeholder="Gestor" oninput="filtrarPainelAnotacoesOperacionaisV82161()">
        <select id="filtroAnotacoesStatusV82161" onchange="filtrarPainelAnotacoesOperacionaisV82161()">
          <option value="">Todos os status</option>
          <option value="Completa">Completa</option>
          <option value="Ação pendente">Ação pendente</option>
          <option value="Sem observação">Sem observação</option>
          <option value="Incompleta">Incompleta</option>
        </select>
        <button class="btn secondary" onclick="limparFiltrosAnotacoesOperacionaisV82161()">Limpar</button>
      </section>
      <div class="table-header op-notes-header">
        <h3>Rastreabilidade das Anotações</h3>
        <span id="resumoAnotacoesOperacionaisV82161"></span>
      </div>
      <div class="table-wrap op-notes-wrap">
        <table class="dashboard-table op-notes-table">
          <thead>
            <tr>
              <th>Guia</th><th>Colaborador</th><th>Data/Período</th><th>Ocorrência</th>
              <th>Nome do Gestor</th><th>Adm Responsável</th><th>Observação</th><th>Status</th><th>Atualização</th>
            </tr>
          </thead>
          <tbody id="tbodyAnotacoesOperacionaisV82161"></tbody>
        </table>
      </div>
    </div>
  `;
...
async function carregarPainelAnotacoesOperacionaisV82161(force = false) {
  const paginaEsperada = "anotacoes_operacionais";
  const tokenEsperado = tokenAtualPagina();
  if (qs("statusCarga")) qs("statusCarga").textContent = "Carregando anotações operacionais...";

  try {
    const anotacoes = await carregarAnotacoesOperacionais(force);
    if (!requisicaoAindaValida(paginaEsperada, tokenEsperado)) return;
    anotacoesOperacionaisPainelListaV82161 = montarListaAnotacoesOperacionaisV82161(anotacoes);
    renderizarPainelAnotacoesOperacionaisV82161(anotacoesOperacionaisPainelListaV82161);
  } catch (e) {
    if (!requisicaoAindaValida(paginaEsperada, tokenEsperado)) return;
    const painel = qs("dashboardExecutivoPremium");
    if (painel) painel.innerHTML = `<div class="exec-empty">Erro ao carregar anotações: ${escapeHtml(e.message)}</div>`;
    if (qs("statusCarga")) qs("statusCarga").textContent = "Erro ao carregar anotações operacionais.";
  }
}

function abrirPainelAnotacoesOperacionaisV82161(el) {
  iniciarTransicaoPagina("anotacoes_operacionais");
  paginaAtual = "anotacoes_operacionais";
  marcarPaginaAtualClasse();
  setVisibilidadeDashboardExecutivoPremium(true);
  dadosConsolidadoAtual = [];
  document.querySelectorAll(".menu button").forEach(b => b.classList.remove("active"));
  if (el) el.classList.add("active");
  if (qs("tituloPagina")) qs("tituloPagina").textContent = "Anotações Operacionais";
  if (qs("subtituloPagina")) qs("subtituloPagina").textContent = "";
  if (qs("tituloTabela")) qs("tituloTabela").textContent = "Anotações Operacionais";
  if (qs("filtros")) qs("filtros").classList.add("hidden");
  if (qs("filtrosDashboard")) qs("filtrosDashboard").classList.add("hidden");
  if (qs("boxFiltroTipo")) qs("boxFiltroTipo").classList.add("hidden");
  if (qs("controleOrdenacaoBancoHoras")) qs("controleOrdenacaoBancoHoras").classList.add("hidden");
  if (qs("tabelaDashboard")) qs("tabelaDashboard").classList.add("hidden");
  if (qs("tabelaConsolidado")) qs("tabelaConsolidado").classList.add("hidden");
  carregarPainelAnotacoesOperacionaisV82161();
}
  ...
  return `
    <div class="exec-rank-row">
      <div class="exec-rank-main">
        ...
        <span>${valorSeguro(r.cc, "")} ${r.funcao ? "â€¢ " + r.funcao : ""} ${r.turno ? "â€¢ " + r.turno : ""}</span>
      </div>
          ...
          return `
            <div class="exec-group-row">
              <div>
                ...
                <span>${valorSeguro(r.colaboradores, 0)} colaborador(es) â€¢ ${valorSeguro(r.registros, 0)} registro(s)</span>
              </div>
              <div class="exec-group-metrics">
                <b>Saldo ${valorSeguro(r.saldo, "00,00")}</b>
                <span>HE ${valorSeguro(r.he, "00:00")} â€¢ Aus. ${valorSeguro(r.absent, "00:00")}</span>
              </div>
              <div class="exec-bar"><i style="width:${pct.toFixed(1)}%"></i></div>
            </div>
          `;
  ...
  return `
    <section class="exec-panel">
      <div class="exec-panel-title">
        ...
        <span class="exec-chip">${anterior.inicio || "-"} a ${anterior.fim || "-"} â†’ ${atual.inicio || "-"} a ${atual.fim || "-"}</span>
      </div>
      <div class="exec-list">
        ...
        ${colaboradores.length ? colaboradores.map(c => `
          <div class="alert-person ${c.classe || "info"}">
            ...
            <div class="alert-person-main">
              ...
              <span>${c.cc || "-"} â€¢ ${c.funcao || "-"} â€¢ ${c.turno || "-"}</span>
...
function renderPainelNotificacoesJornada(status) {
  ...
  const ultimoTexto = ultimo ? `Último envio: ${ultimo.quando} â€” ${ultimo.quantidade} ocorrência(s).` : "Nenhum envio registrado ainda.";
  return `
...
ajustarFiltroTipoInconsistencias();
  ...
  } else if (pagina === "hora_extra_simplificada") {
    paginaAtual = "hora_extra_simplificada";
    renderizarCardsPorGuia();
    qs("tituloPagina").textContent = "Horas Extras Simplificadas";
...
function atualizarDadosGuiaAtual() {
  ...
  if (pagina === "governanca_servidor") return carregarGovernancaServidorV82144();
  ...
  if (pagina === "hora_extra" || pagina === "hora_extra_simplificada" || pagina === "ausencias" || pagina === "inconsistencias" || pagina === "consolidado") return carregarConsolidado();
...
function recarregarPaginaAtualFiltros() {
  ...
  timerFiltroPrincipal = setTimeout(() => {
    ...
    else if (paginaAtual === "governanca_servidor") carregarGovernancaServidorV82144();
...
function elementoEstaVisivelV82160(el) {
  if (!el) return false;
...
function tabelaAtualParaExportacaoV82160() {
  const candidatas = [
    qs("tabelaDashboard"),
    qs("tabelaConsolidado"),
    ...Array.from(document.querySelectorAll("#dashboardExecutivoPremium table, #painelBancoHoras2 table"))
  ].filter(Boolean);

  return candidatas.find(tabela => {
    if (!elementoEstaVisivelV82160(tabela)) return false;
    const linhas = Array.from(tabela.querySelectorAll("tbody tr"));
    return linhas.some(linha => (linha.textContent || "").trim());
  }) || null;
}

function substituirControlesPorTextoV82160(tabela) {
  tabela.querySelectorAll("input, textarea, select").forEach(controle => {
    let valor = "";
    if (controle.tagName === "SELECT") {
      const selecionada = controle.options && controle.selectedIndex >= 0 ? controle.options[controle.selectedIndex] : null;
      valor = selecionada ? selecionada.textContent : controle.value;
    } else {
      valor = controle.value || controle.textContent || "";
    }
    controle.replaceWith(document.createTextNode(valor));
  });

  tabela.querySelectorAll("button").forEach(botao => {
    const texto = (botao.textContent || "").replace(/[↕↑↓]/g, "").trim();
    botao.replaceWith(document.createTextNode(texto));
  });

  tabela.querySelectorAll("[style]").forEach(el => el.removeAttribute("style"));
  tabela.querySelectorAll("script, svg").forEach(el => el.remove());
}

function nomeArquivoExportacaoGuiaV82160() {
  const titulo = (qs("tituloPagina")?.textContent || qs("tituloTabela")?.textContent || paginaAtual || "guia").trim();
  const limpo = titulo
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
  ...
  return `tempo_fechado_${limpo}_${carimbo}.xls`;
}

function exportarGuiaAtualParaExcelV82160() {
  try {
    if (!PAGINAS_EXPORTAVEIS_V82160.has(paginaAtual)) {
      alert("A guia atual não possui tabela de produção para exportar.");
      return;
    }

    const tabela = tabelaAtualParaExportacaoV82160();
    if (!tabela) {
      alert("Nenhuma tabela carregada foi encontrada nesta guia.");
      return;
    }
 ...
 <Worksheet ss:Name="${escapeXml(titulo.slice(0, 31))}">
  <Table>
   ${rowsXml}
    ...
    link.href = url;
    link.download = nomeArquivoExportacaoGuiaV82160();
    document.body.appendChild(link);
    link.click();
    link.remove();
    setTimeout(() => URL.revokeObjectURL(url), 1500);

    if (qs("statusCarga")) qs("statusCarga").textContent = "Guia exportada para Excel.";
  } catch (e) {
    alert("Não foi possível exportar a guia: " + e.message);
  }
}
...
// v8.21.50 - cabecalhos com anotacoes operacionais por linha.
function setCabecalhoInconsistenciasCompacto() {
  const thead = document.querySelector("#tabelaConsolidado thead tr");
  if (!thead) return;
  thead.innerHTML = `
    <th>CC</th><th>Nome</th><th>Data</th><th>Dia</th>
    <th>1a E.</th><th>1a S.</th><th>2a E.</th><th>2a S.</th><th>3a E.</th><th>3a S.</th><th>4a E.</th><th>4a S.</th>
    <th>Jornada</th><th>Observa&ccedil;&atilde;o Original</th><th>Status</th>
    <th><button class="th-sort" onclick="alternarOrdenacaoInconsistencias()">Inconsistencias<span id="sortInconsistenciasIcon"> ↕</span></button></th>
    ${cabecalhoAnotacaoOperacional()}
  `;
}
...
function setCabecalhoSemMarcacaoCompacto() {
  ...
  if (!thead) return;
  thead.innerHTML = `
    <th>CC</th><th>Nome</th><th>Fun&ccedil;&atilde;o</th><th>Turno</th><th>Data</th><th>Dia</th>
    <th>1a E.</th><th>1a S.</th><th>2a E.</th><th>2a S.</th><th>3a E.</th><th>3a S.</th><th>4a E.</th><th>4a S.</th>
    <th>Abono</th><th>Absent.</th><th>Observa&ccedil;&atilde;o Original</th>
    ${cabecalhoAnotacaoOperacional()}
  `;
}
...
function setCabecalhoPontoAbertoCompacto() {
  ...
  thead.innerHTML = `
    <th>CC</th><th>Nome</th><th>Fun&ccedil;&atilde;o</th><th>Turno</th><th>Data</th><th>Dia</th>
    <th>1a E.</th><th>1a S.</th><th>2a E.</th><th>2a S.</th><th>3a E.</th><th>3a S.</th><th>4a E.</th><th>4a S.</th>
    <th>Absent.</th><th>Observa&ccedil;&atilde;o Original</th><th>Status</th><th>Inconsist&ecirc;ncias</th>
    ${cabecalhoAnotacaoOperacional()}
  `;
}
...
function setCabecalhoHoraExtraCompacto() {
  ...
  thead.innerHTML = `
    ...
    <th>1a E.</th><th>1a S.</th><th>2a E.</th><th>2a S.</th><th>3a E.</th><th>3a S.</th><th>4a E.</th><th>4a S.</th>
    <th><button class="th-sort" onclick="alternarOrdenacaoHE()">H.E.<span id="sortHEIcon"> ↕</span></button></th>
    <th>Jornada</th><th>Observa&ccedil;&atilde;o Original</th><th>Status</th><th>Inconsist&ecirc;ncias</th>
    ${cabecalhoAnotacaoOperacional()}
  `;
}
...
function setCabecalhoInterJornada() {
  ...
  if (!thead) return;
  thead.innerHTML = `
    <th>CC</th><th>Nome</th><th>Fun&ccedil;&atilde;o</th><th>Turno</th>
    <th>Data Anterior</th><th>&Uacute;ltima Marca&ccedil;&atilde;o</th><th>Data Retorno</th><th>Primeira Marca&ccedil;&atilde;o</th>
    <th>Intervalo</th><th>D&eacute;ficit</th><th>Observa&ccedil;&atilde;o Original</th>
    ${cabecalhoAnotacaoOperacional()}
  `;
}
...
function setCabecalhoViolacoesJornada() {
  ...
  if (!thead) return;
  thead.innerHTML = `
    <th>CC</th><th>Nome</th><th>Fun&ccedil;&atilde;o</th><th>Tipo Jornada</th><th>Per&iacute;odo</th>
    <th>Data Refer&ecirc;ncia</th><th>Viola&ccedil;&atilde;o</th><th>Detalhe</th><th>Dias Trabalhados</th><th>Limite Esperado</th>
    ${cabecalhoAnotacaoOperacional()}
  `;
}
...
function renderInconsistenciasCompacta(rows) {
  ...
  const renderLinha = (r) => `
    <tr>
      ...
      <td>${statusBadge(r.status)}</td>
      <td>${r.inconsistencias ?? ""}</td>
      ${renderCamposAnotacaoOperacional("inconsistencias", r)}
    </tr>
  `;
...
function renderSemMarcacaoCompacta(rows) {
  ...
  const renderLinha = (r) => `
    <tr>
      ...
      <td>${r.absent ? badge(r.absent, "danger") : ""}</td>
      <td>${r.observacao ?? ""}</td>
      ${renderCamposAnotacaoOperacional("sem_marcacao", r)}
    </tr>
  `;
...
function renderPontoAbertoCompacto(rows) {
  ...
  const renderLinha = (r) => `
    <tr>
      ...
      <td>${statusBadge(r.status)}</td>
      <td>${r.inconsistencias ?? ""}</td>
      ${renderCamposAnotacaoOperacional("ponto_aberto", r)}
    </tr>
  `;
...
function renderHoraExtraCompacta(rows) {
  ...
  const renderLinha = (r) => `
    <tr>
      ...
      <td>${statusBadge(r.status)}</td>
      <td>${r.inconsistencias ?? ""}</td>
      ${renderCamposAnotacaoOperacional("horas_extras", r)}
    </tr>
  `;
...
function renderInterJornada(rows) {
  ...
  const renderLinha = (r) => `
    <tr>
      ...
      <td>${r.deficit_interjornada ?? ""}</td>
      <td>${r.observacao ?? ""}</td>
      ${renderCamposAnotacaoOperacional("inter_jornada", r)}
    </tr>
  `;
...
function renderViolacoesJornada(rows) {
  ...
  const renderLinha = (r) => `
    <tr>
      ...
      <td>${r.dias_trabalhados ?? ""}</td>
      <td>${r.limite_esperado ?? ""}</td>
      ${renderCamposAnotacaoOperacional("violacoes_jornada", r)}
    </tr>
  `;
...
function limparFiltros() {
  ...
  ordenacaoInconsistencias = null;
  if (qs("tipo")) {
    if (paginaAtual === "hora_extra" || paginaAtual === "hora_extra_simplificada") qs("tipo").value = "he";
    if (paginaAtual === "ausencias") qs("tipo").value = "ausencia";
  }
  recarregarPaginaAtualFiltros();
}
...
function renderProcessamentoStatus(job) {
  ...
  const launcherStatus = launcher.rotulo || (launcher.ok === undefined ? "Aguardando" : (launcher.ok ? "OK" : "Erro"));
  const launcherMensagem = launcher.mensagem || (launcher.caminho ? "Componente de coleta localizado." : "");
  const excelStatus = imp.executado === false ? "Nao executada" : (imp.ok === undefined ? "Aguardando" : (imp.ok ? "Importado" : "Erro"));
  const travaHtml = renderTravaOperacionalV82140(job.trava_operacional);
  const filaHtml = renderHistoricoFilaV82141(job.historico_fila);
  alvo.innerHTML = `
    ${travaHtml}
    ...
    <div class="process-mini-grid">
      <div><strong>Coleta</strong><span>${escapeHtml(launcherStatus)}</span><small>${escapeHtml(launcherMensagem)}</small></div>
      <div><strong>Excel</strong><span>${escapeHtml(excelStatus)}</span><small>${escapeHtml(imp.mensagem || "")}</small></div>
...
function renderTravaOperacionalV82140(trava) {
  trava = trava || {};
  if (!trava.ativa) return "";
  const nome = trava.nome || trava.usuario || "usuario";
  const tipo = trava.tipo || "Processamento";
  const inicio = trava.iniciado_em || "-";
  const mensagem = trava.mensagem || "Processamento em andamento.";
  const recuperada = trava.recuperada_apos_reinicio ? `<span>Recuperada apos reinicio</span>` : "";
  return `
    <div class="process-status-card warning trava-operacional-card-v82140">
      <div>
        <span class="exec-eyebrow">Trava operacional</span>
        <h4>${escapeHtml(tipo)} em execução</h4>
      ...
      <div class="process-status-meta">
        ...
        ${recuperada}
        <button class="btn secondary requires-admin" type="button" onclick="liberarTravaProcessamentoV82140()">Liberar trava</button>
      </div>
    </div>
  `;
}

function renderHistoricoFilaV82141(eventos) {
  eventos = Array.isArray(eventos) ? eventos : [];
  if (!eventos.length) return "";
  return `
    <details class="process-log-box fila-historico-v82141">
      <summary>Histórico da fila</summary>
      <div class="process-mini-grid">
        ${eventos.slice(0, 8).map(e => {
          const trava = e.trava || {};
          const quem = trava.nome || trava.usuario || "";
          const tipo = trava.tipo || e.evento || "Fila";
          const msg = `${e.quando || ""} ${quem ? quem + " - " : ""}${e.mensagem || ""}`;
          return `<div><strong>${escapeHtml(tipo)}</strong><span>${escapeHtml(e.status || e.evento || "-")}</span><small>${escapeHtml(msg)}</small></div>`;
        }).join("")}
      </div>
    </details>
  `;
...
async function liberarTravaProcessamentoV82140() {
  if (!usuarioAtualEhAdmin()) {
  ...
  try {
    ...
    alert(json.mensagem || "Trava liberada.");
    const atualizacoes = [carregarPainelCicloOperacional()];
    if (paginaAtual === "processamento_manual_robo") atualizacoes.push(abrirProcessamentoManualRoboV81737());
    else atualizacoes.push(consultarStatusProcessamentoIntegrado());
    await Promise.allSettled(atualizacoes);
  } catch (e) {
    alert("Falha ao liberar trava: " + e.message);
  }
}
  ...
  try {
    ...
    if (job.status !== "rodando" && timerProcessamentoIntegrado) {
      ...
      if (job.status === "concluido") {
        ...
        if (qs("statusCarga")) qs("statusCarga").textContent = "Processamento integrado terminou com erro. Veja os detalhes da execução.";
      }
    }
    return job;
  } catch (e) {
...
async function iniciarProcessamentoIntegrado() {
  if (!usuarioPodeV82143("processar")) {
    alert("Acesso negado. Esta acao exige perfil Supervisor ou Administrador.");
    return;
  }
  const payload = {
...
function valorMarcacaoHoraExtraSimplificadaV82167(valor) {
  ...
  return texto && !["nan", "none", "nat", "undefined"].includes(texto.toLowerCase()) ? texto : "";
}

function primeiraEntradaHoraExtraSimplificadaV82167(r) {
  for (const campo of ["e1", "e2", "e3", "e4"]) {
    const valor = valorMarcacaoHoraExtraSimplificadaV82167(r?.[campo]);
    if (valor) return valor;
  }
  return "-";
}

function ultimaSaidaHoraExtraSimplificadaV82167(r) {
  for (const campo of ["s4", "s3", "s2", "s1"]) {
    const valor = valorMarcacaoHoraExtraSimplificadaV82167(r?.[campo]);
    if (valor) return valor;
  }
  return "-";
...
function renderCamposHoraExtraSimplificadaV82167(r) {
  const guia = "horas_extras_simplificadas";
  const chave = [guia, r?.cc, r?.nome, r?.data || r?.data_referencia || "", r?.dia || ""]
...
function renderHoraExtraSimplificadaV82167(rows) {
  const linhas = limitarRenderTabela(aplicarOrdenacao(rows || []), "registros simplificados de hora extra");
  const paginaRenderizada = paginaAtual;
  const renderLinha = (r) => `
    <tr>
      ...
      <td>${r.he ? badge(escapeHtml(r.he), "warning") : ""}</td>
      ${renderCamposHoraExtraSimplificadaV82167(r)}
    </tr>
  `;

  atualizarIndicadoresOrdenacao();
  qs("statusCarga").textContent = linhas.length ? `Renderizando ${linhas.length} registro(s) simplificado(s) de hora extra...` : "Nenhum registro de hora extra encontrado.";
  renderTabelaEmLotes("tbody", linhas, renderLinha, {
    aoConcluir: () => {
...
async function uploadGuiaHoraExtraSimplificadaV82172() {
  ...
  input.type = "file";
  input.accept = ".xls,.xlsx,.xlsm,.csv,.htm,.html,.txt,.xml";
  ...
  input.onchange = async (e) => {
    try {
      ...
      if (qs("statusCarga")) {
        qs("statusCarga").textContent = "Processando upload da planilha...";
      }
...
function setVisibilidadeDashboardHorasExtras(mostrar) {
  ...
  const cards = qs("cardsContainer");
  if (painel) painel.classList.toggle("hidden", !mostrar);
  if (wrap) wrap.classList.toggle("hidden", mostrar);
  if (tableHeader) tableHeader.classList.toggle("hidden", mostrar);
  if (cards && mostrar) cards.classList.add("hidden");
  if (mostrar) {
...
function ocultarDashboardHorasExtras() {
  setVisibilidadeDashboardHorasExtras(false);
}
...
function abrirDashboardHorasExtras(el) {
  ...
  if (qs("tituloPagina")) qs("tituloPagina").textContent = "Dashboard de Gestão de Horas Extras";
  if (qs("subtituloPagina")) qs("subtituloPagina").textContent = "Indicadores consolidados das folhas de ponto e horas extras.";
...
function processarDadosDashboardHorasExtras(linhas) {
  ...
  return {
    ...
    maioresOcorrencias: ocorrenciasDetalhadas.slice(0, 20)
  };
}
...
function renderDashboardHorasExtrasVazio() {
  ...
  painel.innerHTML = `
    <div class="os-dash-header">
      <div>
        <h1 class="os-dash-title">Dashboard de Gestão de Horas Extras</h1>
        <p class="os-dash-subtitle">Nenhum registro de hora extra encontrado para os filtros selecionados.</p>
      </div>
    </div>
    <div class="os-dash-body">
      <p class="os-empty-text" style="text-align:center;padding:40px 0;">Ajuste os filtros acima (período, CC ou colaborador) e tente novamente.</p>
    </div>
  `;
}
...
function renderDashboardHorasExtras(stats) {
  ...
  painel.innerHTML = `
    <!-- Topbar idêntica ao Dashboard de OS -->
    <div class="os-dash-header">
      <div>
        <h1 class="os-dash-title">Dashboard de Gestão de Horas Extras</h1>
        <p class="os-dash-subtitle">Indicadores consolidados das folhas de ponto — Período: ${escapeHtml(textoPeriodo)}</p>
      </div>
      <div class="os-dash-actions">
        <input
          id="heDashboardSearchInput"
          type="text"
          placeholder="Filtrar colaboradores..."
          oninput="filtrarDashboardHorasExtras(this.value)"
          class="os-dash-search"
        />
        <button
          type="button"
          onclick="exportarDashboardHorasExtrasPdf()"
          class="btn-os-pdf"
        >
          Gerar PDF
        </button>
    ...
    <div class="os-dash-body">
      <!-- 4 KPI Cards idênticos ao padrão do Dashboard de OS -->
      <div class="os-kpi-grid">
        <div class="os-kpi-card">
          <p class="os-kpi-label">Total de Horas Extras</p>
          <p class="os-kpi-val" style="color:#d97706;">${k.totalHorasFormatado}</p>
          <div class="os-kpi-meta">${k.totalHorasDecimais}h decimais no período</div>
        </div>
        <div class="os-kpi-card">
          <p class="os-kpi-label">Colaboradores</p>
          <p class="os-kpi-val" style="color:#2563eb;">${k.totalColabs}</p>
          <div class="os-kpi-meta">Média de ${k.mediaHorasFormatado} / colaborador</div>
        </div>
        <div class="os-kpi-card">
          <p class="os-kpi-label">Acionamentos Sobreaviso</p>
          <p class="os-kpi-val" style="color:#7c3aed;">${k.sobreavisoSimCount}</p>
          <div class="os-kpi-meta">${k.sobreavisoSimHoras} (${k.sobreavisoSimPct}% do total)</div>
        </div>
        <div class="os-kpi-card">
          <p class="os-kpi-label">Aderência Justificativas</p>
          <p class="os-kpi-val" style="color:#059669;">${k.taxaJustificadas}%</p>
          <div class="os-kpi-meta">${k.justificadasCount} justif. / ${k.pendentesCount} pendentes</div>
        </div>
      </div>

      <!-- Grid 2 Colunas: Top Colaboradores + Dias da Semana -->
      <div class="os-grid-2">
        <div class="os-card">
          <h3 class="os-card-title">Top 10 Colaboradores com Mais Horas Extras</h3>
          <div id="osColaboradoresBars" class="os-bar-stack">
            ${renderBreakdownBarsColaboradores(stats.topColaboradores, k.totalMinutos)}
          </div>
        </div>

        <div class="os-card">
          <h3 class="os-card-title">Horas Extras por Dia da Semana</h3>
          <div id="osDiasSemanaBars" class="os-bar-stack">
            ${renderBreakdownBarsDiasSemana(stats.diasSemana, k.totalMinutos)}
          </div>
        </div>
      </div>

      <!-- Grid 2 Colunas: Centros de Custo + Gestores Mediatos -->
      <div class="os-grid-2">
        <div class="os-card">
          <h3 class="os-card-title">Horas Extras por Centro de Custo</h3>
          <div id="osCentrosCustoBars" class="os-bar-stack">
            ${renderBreakdownBarsCentrosCusto(stats.topCcs, k.totalMinutos)}
          </div>
        </div>

        <div class="os-card">
          <h3 class="os-card-title">Horas Extras por Gestor Mediato</h3>
          <div id="osGestoresBars" class="os-bar-stack">
            ${renderBreakdownBarsGestores(stats.topGestores, k.totalMinutos)}
          </div>
        </div>
      </div>

      <!-- Gráfico de Evolução Diária (Largura Total, estilo OS) -->
      <div class="os-card">
        <h3 class="os-card-title">Evolução Diária de Horas Extras</h3>
        <div style="width:100%;overflow-x:auto;">
          ${renderSvgEvolucaoHorasExtras(stats.evolucao, k.totalMinutos)}
        </div>
      </div>

      <!-- Tabela de Maiores Ocorrências (Largura Total, estilo OS) -->
      <div class="os-card">
        <h3 class="os-card-title">Maiores Ocorrências de Horas Extras no Período</h3>
        <div id="osTabelaOcorrenciasWrap" style="overflow-x:auto;">
...
// ------------------------------------------------------------
// BARRAS DE BREAKDOWN (MODELO DASHBOARD DE OS)
// ------------------------------------------------------------

function renderBreakdownBarsColaboradores(topColaboradores, totalMinutos) {
  if (!topColaboradores || !topColaboradores.length) {
    return '<p class="os-empty-text">Nenhum colaborador com horas extras.</p>';
  }
  const maxMin = topColaboradores[0]?.minutos || 1;
  const colors = ["#D97706", "#2563EB", "#0284C7", "#3B82F6", "#60A5FA", "#818CF8", "#A78BFA", "#94A3B8", "#94A3B8", "#94A3B8"];
  ...
  for (let i = 0; i < topColaboradores.length; i++) {
    ...
    const barPct = Math.max(Math.round((item.minutos / maxMin) * 100), 2);
    const color = colors[i % colors.length];
    const horasStr = formatarMinutosParaHorasMinutos(item.minutos);

    html += `
      <div class="os-bar-row">
        <span class="os-bar-dot" style="background:${color}"></span>
        <span class="os-bar-label" title="${escapeHtml(item.nome)}">${i + 1}. ${escapeHtml(item.nome)}</span>
        <div class="os-bar-track">
          <div class="os-bar-fill" style="width:${barPct}%;background:${color}"></div>
        </div>
        <span class="os-bar-val">${horasStr} <small>(${pct}%)</small></span>
      </div>
    `;
  }
  return html;
...
function renderBreakdownBarsDiasSemana(diasSemana, totalMinutos) {
  if (!diasSemana || !diasSemana.length) {
    return '<p class="os-empty-text">Sem dados por dia da semana.</p>';
  }
  let maxMin = 1;
  ...
  for (let i = 0; i < diasSemana.length; i++) {
    ...
    const pct = totalMinutos > 0 ? ((d.minutos / totalMinutos) * 100).toFixed(1) : "0";
    const barPct = Math.max(Math.round((d.minutos / maxMin) * 100), 2);
    const isWeekend = d.label === "Sáb" || d.label === "Dom";
    const color = isWeekend ? "#8B5CF6" : "#3B82F6";
    const horasStr = formatarMinutosParaHorasMinutos(d.minutos);

    html += `
      <div class="os-bar-row">
        <span class="os-bar-dot" style="background:${color}"></span>
        <span class="os-bar-label" style="font-weight:${isWeekend ? "700" : "500"};color:${isWeekend ? "#7C3AED" : "#475569"};">${escapeHtml(d.label)}</span>
        <div class="os-bar-track">
          <div class="os-bar-fill" style="width:${barPct}%;background:${color}"></div>
        </div>
        <span class="os-bar-val">${horasStr} <small>(${pct}%)</small></span>
...
function renderBreakdownBarsCentrosCusto(topCcs, totalMinutos) {
  if (!topCcs || !topCcs.length) {
    return '<p class="os-empty-text">Nenhum Centro de Custo registrado.</p>';
  }
  ...
  for (let i = 0; i < topCcs.length; i++) {
    ...
    html += `
      <div class="os-bar-row">
        <span class="os-bar-dot" style="background:${color}"></span>
        <span class="os-bar-label" title="${escapeHtml(item.cc)}">${escapeHtml(item.cc)}</span>
        <div class="os-bar-track">
          <div class="os-bar-fill" style="width:${barPct}%;background:${color}"></div>
        </div>
        <span class="os-bar-val">${horasStr} <small>(${pct}%)</small></span>
...
function renderBreakdownBarsGestores(topGestores, totalMinutos) {
  if (!topGestores || !topGestores.length) {
    return '<p class="os-empty-text">Nenhum Gestor Mediato registrado.</p>';
  }
  ...
  for (let i = 0; i < topGestores.length; i++) {
    ...
    html += `
      <div class="os-bar-row">
        <span class="os-bar-dot" style="background:${color}"></span>
        <span class="os-bar-label" title="${escapeHtml(item.gestor)}">${escapeHtml(item.gestor)}</span>
        <div class="os-bar-track">
          <div class="os-bar-fill" style="width:${barPct}%;background:${color}"></div>
        </div>
        <span class="os-bar-val">${horasStr} <small>(${pct}%)</small></span>
...
function filtrarDashboardHorasExtras(termo) {
  if (!_dashboardHorasExtrasEstatisticas) return;
  const termoNorm = String(termo || "").trim().toLowerCase();
  const todos = _dashboardHorasExtrasEstatisticas.maioresOcorrencias || [];

  const filtrados = termoNorm
    ? todos.filter(r =>
        (r.nome && r.nome.toLowerCase().includes(termoNorm)) ||
        (r.cc && r.cc.toLowerCase().includes(termoNorm)) ||
        (r.gestor && r.gestor.toLowerCase().includes(termoNorm)) ||
        (r.observacao && r.observacao.toLowerCase().includes(termoNorm))
      )
    : todos;

  const alvo = qs("osTabelaOcorrenciasWrap");
  if (alvo) {
    alvo.innerHTML = renderTabelaMaioresOcorrenciasHe(filtrados);
  }
}

function renderTabelaMaioresOcorrenciasHe(ocorrencias) {
  if (!ocorrencias || !ocorrencias.length) {
    return '<p class="os-empty-text">Nenhuma ocorrência encontrada.</p>';
  }

  let html = `
    <table class="os-dash-table">
      <thead>
        <tr>
          <th>Data</th>
          <th>Dia</th>
  ...
  for (const r of ocorrencias) {
    ...
    const badgeSob = isSobreaviso ? '<span class="os-badge sim">Sim</span>' : '<span class="os-badge nao">Não</span>';
    const hasJust = Boolean(r.observacao && r.observacao.trim());
    const badgeJust = hasJust
      ? `<span class="os-badge justificado" title="${escapeHtml(r.observacao)}">${escapeHtml(r.observacao.slice(0, 45))}${r.observacao.length > 45 ? "..." : ""}</span>`
      : '<span class="os-badge pendente">Pendente</span>';

    html += `
      <tr>
        <td><strong>${escapeHtml(r.data)}</strong></td>
        <td>${escapeHtml(r.dia)}</td>
        <td><strong style="color:#0f172a;">${escapeHtml(r.nome)}</strong></td>
        <td>${escapeHtml(r.cc)}</td>
...
// ------------------------------------------------------------
// SVG CHARTS BUILDERS (VETORIAIS NATIVOS MODELO OS DASHBOARD)
// ------------------------------------------------------------

function renderSvgEvolucaoHorasExtras(evolucao, totalMinutos) {
  if (!evolucao || !evolucao.length) {
    return '<p class="os-empty-text">Sem dados de evolução no período.</p>';
  }

  const W = 820, H = 220, P = { t: 20, r: 24, b: 40, l: 44 };
  ...
  // Pontos com estilo OS Dashboard
  let dots = "";
  for (let i = 0; i < pts.length; i++) {
    dots += `<circle cx="${pts[i].x.toFixed(1)}" cy="${pts[i].y.toFixed(1)}" r="3.5" fill="#1E3A5F" stroke="#fff" stroke-width="1.5"/>`;
  }
  ...
  return `
    <svg viewBox="0 0 ${W} ${H}" style="width:100%;height:auto;display:block;" xmlns="http://www.w3.org/2000/svg">
      ...
      <path d="${line}" fill="none" stroke="#1E3A5F" stroke-width="2.5" stroke-linejoin="round" stroke-linecap="round"/>
      ${dots}
      ${xLabels}
    </svg>
  `;
...
function exportarDashboardHorasExtrasPdf() {
  ...
  const barsColabs = renderBreakdownBarsColaboradores(stats.topColaboradores, k.totalMinutos);
  const barsDias = renderBreakdownBarsDiasSemana(stats.diasSemana, k.totalMinutos);
  const barsCcs = renderBreakdownBarsCentrosCusto(stats.topCcs, k.totalMinutos);
  const barsGestores = renderBreakdownBarsGestores(stats.topGestores, k.totalMinutos);
...
<head>
  <meta charset="utf-8">
  <title>Dashboard de Gestão de Horas Extras — Relatório Executivo</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .header h1 { font-size: 20px; color: #0f172a; font-weight: 800; }
    .header p { font-size: 11px; color: #64748b; margin-top: 3px; }
    .header .meta { text-align: right; font-size: 11px; color: #475569; }
    .section { margin-bottom: 20px; page-break-inside: avoid; }
    .section-title { font-size: 13px; font-weight: 700; color: #0f172a; margin-bottom: 10px; padding-bottom: 4px; border-bottom: 1px solid #e2e8f0; }
    ...
    .os-card { border: 1px solid #e2e8f0; border-radius: 12px; padding: 14px; background: #fff; }
    .os-bar-stack { display: flex; flex-direction: column; gap: 10px; }
    .os-bar-row { display: flex; align-items: center; gap: 10px; font-size: 11px; }
    .os-bar-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
    .os-bar-label { width: 140px; flex-shrink: 0; color: #475569; font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .os-bar-track { flex: 1; background: #f1f5f9; border-radius: 999px; height: 12px; overflow: hidden; }
    .os-bar-fill { height: 12px; border-radius: 999px; }
    .os-bar-val { width: 90px; text-align: right; flex-shrink: 0; color: #334155; font-weight: 600; font-size: 11px; }
    .os-dash-table { width: 100%; border-collapse: collapse; font-size: 10.5px; }
    .os-dash-table th { background: #0f172a; color: #fff; padding: 6px 8px; text-align: left; font-size: 9.5px; text-transform: uppercase; }
    .os-dash-table td { padding: 5px 8px; border-bottom: 1px solid #e2e8f0; }
    .os-badge { display: inline-block; padding: 2px 6px; border-radius: 999px; font-size: 9px; font-weight: 700; }
    .os-badge.sim { background: #ede9fe; color: #6d28d9; }
    .os-badge.nao { background: #f1f5f9; color: #64748b; }
    .os-badge.justificado { background: #d1fae5; color: #047857; }
    .os-badge.pendente { background: #fee2e2; color: #b91c1c; }
    @page { margin: 1cm; }
...
<body>
  <div class="header">
    <div>
      <h1>Dashboard de Gestão de Horas Extras</h1>
  ...
  <div class="section">
    <div class="two-col">
      <div class="os-card">
        ...
        <div class="os-bar-stack">${barsColabs}</div>
      </div>
      <div class="os-card">
        <div class="section-title">Distribuição por Dia da Semana</div>
        <div class="os-bar-stack">${barsDias}</div>
      </div>
    </div>
  </div>
  ...
  <div class="section">
    <div class="two-col">
      <div class="os-card">
        ...
        <div class="os-bar-stack">${barsCcs}</div>
      </div>
      <div class="os-card">
        <div class="section-title">Horas Extras por Gestor Mediato</div>
        <div class="os-bar-stack">${barsGestores}</div>
      </div>
    </div>
  </div>
...
function renderResultadoFechamentoPeriodoV82166(json) {
  ...
  alvo.innerHTML = `
    <div class="process-status-card ${erros.length ? "warning" : "success"}">
      <div>
        <span class="exec-eyebrow">Fechamento do período</span>
        <h4>${escapeHtml(json.mensagem || "Fechamento arquivado")}</h4>
        <p>${escapeHtml(destino)}</p>
      </div>
      <div class="process-status-meta">
        <strong>${erros.length ? "PARCIAL" : "OK"}</strong>
        <span>${escapeHtml(String(resumo.arquivos || 0))} arquivo(s)</span>
        <span>${escapeHtml(String(Math.round((resumo.bytes || 0) / 1024)))} KB preservados</span>
      </div>
    </div>
    <details class="process-log-box" open>
      <summary>Itens preservados</summary>
      <pre>${escapeHtml((json.itens || []).slice(0, 160).map(x => `${x.categoria}: ${x.destino}`).join("\n") || "Nenhum arquivo operacional encontrado para arquivar.")}</pre>
    </details>
    ${erros.length ? `<details class="process-log-box" open><summary>Itens não preservados</summary><pre>${escapeHtml(erros.map(x => `${x.categoria}: ${x.origem} - ${x.erro}`).join("\n"))}</pre></details>` : ""}
  `;
}

async function executarFechamentoPeriodoV82166() {
  if (!usuarioAtualEhAdmin()) {
    alert("Acesso negado. Esta acao exige perfil Administrador.");
    return;
  }
  ...
  const periodo = prompt("Informe o período a arquivar no formato AAAA-MM:", padrao);
  if (periodo === null) return;
  const periodoLimpo = String(periodo || "").trim();
  if (!/^\d{4}-\d{2}$/.test(periodoLimpo)) {
    alert("Informe o período no formato AAAA-MM. Exemplo: 2026-07.");
    return;
  }
  const observacao = prompt("Observação do fechamento, opcional:", "") || "";
  const ok = confirm(
    "Arquivar fechamento do período " + periodoLimpo + "?\n\n" +
    "O Tempo Fechado vai copiar a base ativa, PDFs, planilhas, anotações e históricos para backup. Nada será apagado."
  );
  if (!ok) return;
  try {
    ...
    const json = await fetchJsonSeguro("/tempo-fechado/api/processamento/fechamento-periodo", {
      ...
      headers: { "Content-Type": "application/json", "Accept": "application/json" },
      credentials: "same-origin",
      body: JSON.stringify({ periodo: periodoLimpo, observacao }),
    });
    renderResultadoFechamentoPeriodoV82166(json);
    await Promise.allSettled([carregarPainelCicloOperacional(), carregarHistoricoProcessamento()]);
    if (qs("statusCarga")) qs("statusCarga").textContent = json.mensagem || "Fechamento arquivado.";
    alert((json.mensagem || "Fechamento arquivado.") + "\n\nPasta:\n" + (json.destino || ""));
  } catch (e) {
    alert("Falha ao arquivar fechamento: " + e.message);
    if (qs("statusCarga")) qs("statusCarga").textContent = "Falha ao arquivar fechamento.";
  }
}
...
async function executarCleanupOperacional() {
  ...
  const itens = [
    ...
    opcoes.limpar_cache_bh ? "dados temporários de Banco de Horas e PDFs" : null,
...
function renderPainelCicloOperacional(json) {
  ...
  alvo.innerHTML = `
    <div class="process-mini-grid">
      <div><strong>Base de dados</strong><span>${base.excel_existe ? "Carregada" : "Vazia"}</span><small>${escapeHtml((base.excel_app || {}).idade || "")}</small></div>
      <div><strong>Entrada</strong><span>${escapeHtml(String(((robo.entrada || {}).pdfs) || 0))} PDF(s)</span><small>Arquivos aguardando tratamento</small></div>
      <div><strong>Saída</strong><span>${escapeHtml(String(((robo.saida || {}).planilhas) || 0))} arquivo(s)</span><small>${escapeHtml((robo.excel || {}).idade || "Última planilha processada")}</small></div>
      <div><strong>Erros</strong><span>${escapeHtml(String(((robo.erro || {}).pdfs) || 0))} PDF(s)</span><small>Arquivos que exigem revisão</small></div>
      <div><strong>Dados temporários</strong><span>${cacheOk} ativo(s)</span><small>Banco de Horas e PDFs</small></div>
...
function renderHistoricoProcessamento(json) {
  ...
  const filaHtml = renderHistoricoFilaV82141(json.historico_fila);
  ...
  alvo.innerHTML = historicoHtml + filaHtml;
}
  ...
  return `
    <div class="process-toolbar">
      ...
      <div class="process-toolbar-actions">
        <button class="btn requires-supervisor" onclick="iniciarProcessamentoIntegrado()">Executar coleta</button>
        <button class="btn requires-admin" onclick="executarNovaRodadaOperacional()">Nova Rodada</button>
        <button class="btn secondary requires-admin" onclick="executarFechamentoPeriodoV82166()">Arquivar período</button>
        <button class="btn secondary" onclick="executarPrevalidacaoProcessamento()">Pre-validar</button>
        <button class="btn secondary requires-admin" onclick="executarCleanupOperacional()">Cleanup</button>
        ...
        <button class="btn secondary requires-admin" onclick="abrirConfiguracaoRobo()">Configurar</button>
      </div>
    </div>
    <section class="exec-panel process-panel-compact">
      <div class="exec-panel-title"><div><h4>Painel de Ciclo</h4><p>Base carregada, arquivos de entrada, saída e processo atual.</p></div></div>
      <div id="cicloOperacionalBox" class="processamento-status-box"><div class="exec-empty">Carregando ciclo operacional...</div></div>
    </section>
    <section class="exec-panel process-panel-compact">
      <div class="exec-panel-title"><div><h4>Fechamento do Período</h4><p>Preserve a fotografia operacional antes de limpar ou carregar novos PDFs.</p></div></div>
      <div class="process-mini-grid cleanup-options-grid">
        <div><strong>Arquivo mensal</strong><span>Backup preservado</span><small>Base ativa, PDFs, planilhas, anotações e históricos.</small></div>
        <div><strong>Destino</strong><span>backup\\fechamentos</span><small>Pastas separadas por período e data de geração.</small></div>
      </div>
    </section>
    <section class="exec-panel process-panel-compact">
      ...
      <div class="process-mini-grid cleanup-options-grid">
        ...
        <label><input id="cleanupBaseApp" type="checkbox" checked> Tabelas da plataforma</label>
        <label><input id="cleanupCacheBh" type="checkbox" checked> Dados temporários Banco de Horas</label>
...
async function abrirProcessamentoIntegrado(btn) {
  ...
  if (!usuarioPodeV82143("processar")) {
    alert("Acesso negado. Esta area exige perfil Supervisor ou Administrador.");
    return;
  }
      ...
      painel.innerHTML = `
        ...
        <section class="exec-panel">
          ...
          <div class="admin-user-form">
            ...
            <select id="adminPerfil"><option value="consulta">consulta</option><option value="operador">operador</option><option value="supervisor">supervisor</option><option value="admin">admin</option></select>
...
async function corrigirBancoHorasPelosPdfs() {
  ...
  function renderStatus(json, tentativa) {
    ...
    alvo.innerHTML = `
      ...
      ${resultado.cache_gerado_em || json.cache_gerado_em ? `Dados atualizados em: ${escapeHtml(resultado.cache_gerado_em || json.cache_gerado_em)}<br>` : ""}
...
async function abrirConfiguracaoRobo(btn) {
  ...
  qs("tituloPagina").textContent = "Configuração da Coleta";
  qs("subtituloPagina").textContent = "";
  marcarPaginaAtualClasse();
  setVisibilidadeDashboardExecutivoPremium(true);
  if (qs("filtros")) qs("filtros").classList.add("hidden");
      ...
      painel.innerHTML = `
        <section class="exec-panel interface-card-compact-v81746">
          <div class="admin-config-grid">
            ...
            <label>Componente de coleta
              <input id="cfgScriptLauncher" value="${escapeHtml(cfgRobo.script_launcher_robo || '')}" placeholder="Opcional no pacote instalado" />
              <small class="field-help">Opcional quando o componente padrão já estiver no pacote instalado.</small>
            </label>
          ...
          <div class="admin-config-actions">
            <button class="btn secondary" onclick="testarConfiguracaoRobo()">Verificar configuração</button>
          ...
          <div class="admin-config-hint">Ambiente verificado para uso operacional.</div>
        </section>
      `;
    }
  } catch (e) {
...
function renderScoreLinha(c){
  ...
  return `<div class="score-row ${scoreBadgeClasse(c.classe)}">
    ...
    <div class="score-main">
      ...
      <span>${escapeHtml(c.cc || "-")} â€¢ ${escapeHtml(c.turno || "-")} â€¢ Banco ${escapeHtml(c.saldo || "00,00")} â€¢ H.E. ${escapeHtml(c.he || "00:00")}</span>
    ...
    <div class="admin-robo-shell">
      ...
      <div class="admin-robo-layout">
        <aside class="admin-robo-tree">
          <button class="active" onclick="selecionarAdminRoboSecaoV81728('launcher', this)">Coleta</button>
        ...
        <section class="admin-robo-content">
          <div class="admin-robo-section" data-admin-robo-section="launcher">
            <h4>Coleta</h4>
            <p>Componente usado no processamento integrado.</p>
            ${campoAdminRoboV81728("adminRoboLauncherPath", "Componente de coleta", launcher.script_launcher_robo || "", "text", "Informativo nesta versão.")}
            <div class="admin-robo-note">Configuração operacional registrada.</div>
          </div>
...
function renderProcessamentoManualRoboV81737(status) {
  ...
  const travaHtml = renderTravaOperacionalV82140(status.trava_operacional);
  alvo.innerHTML = `
    <div class="manual-robo-shell">
      ...
      <div class="manual-robo-grid">
        <section class="exec-panel"><div class="exec-panel-title"><div><h4>Upload de PDFs</h4><p>Carregue os arquivos para a entrada do robô.</p></div></div><input id="manualRoboPdfUpload" type="file" accept="application/pdf,.pdf" multiple /><div class="manual-robo-actions"><button class="btn requires-operador" type="button" onclick="uploadPdfsRoboManualV82138()">Enviar PDFs</button></div><div id="manualRoboUploadStatus" class="admin-robo-note"></div></section>
        <section class="exec-panel"><div class="exec-panel-title"><div><h4>Arquivos operacionais</h4><p>Estrutura de trabalho da coleta.</p></div></div><div class="admin-robo-note"><strong>Entrada:</strong> pasta de PDFs para processamento</div><div class="admin-robo-note"><strong>Saída:</strong> pasta de planilhas geradas</div><div class="admin-robo-note"><strong>Integração:</strong> ${status.script_existe ? "Disponível" : "Pendente"}</div><div class="admin-robo-note"><strong>Planilha:</strong> ${status.excel_existe ? "Gerada" : "Aguardando geração"}</div></section>
        <section class="exec-panel interface-card-compact-v81746"><div class="exec-panel-title"><div><h4>Status</h4></div></div><div class="kpi-grid"><div class="kpi-card"><span>Arquivos de entrada</span><strong>${status.qtd_pdfs_entrada || 0}</strong></div><div class="kpi-card"><span>Integração disponível</span><strong>${status.script_existe ? "Sim" : "Não"}</strong></div><div class="kpi-card"><span>Excel gerado</span><strong>${status.excel_existe ? "Sim" : "Não"}</strong></div></div><div class="manual-robo-actions"><button type="button" class="btn" onclick="abrirPastaEntradaRoboV81740()">Abrir entrada</button><button class="btn requires-supervisor" onclick="importarExcelRoboManualV81737()">Importar Excel</button><button class="btn" onclick="verLogRoboManualV81737()">Ver detalhes</button><button class="btn" onclick="abrirProcessamentoManualRoboV81737()">Recarregar</button></div></section>
      </div>
...
// ============================================================
// v8.21.49 - Painel de Implantação
// ============================================================
...
// ============================================================
// v8.21.49 - Diagnóstico Pós-Instalação
// ============================================================
...
function renderDiagnosticoInstalacaoV82128(payload) {
  ...
  painel.innerHTML = `
    ...
    <div class="exec-kpi-grid">
      ...
      ${cardResumoExecutivo("Erros", resumo.erros ?? 0, "bloqueios operacionais", (resumo.erros || 0) > 0 ? "negative" : "positive")}
      ${cardResumoExecutivo("Sistema", payload.pronto ? "OK" : "Atenção", "status operacional", payload.pronto ? "positive" : "warning")}
    </div>
    ...
    <section class="exec-panel install-diag-panel">
      <div class="exec-panel-title">
        <div>
          ...
          <p>Pacote instalado, pastas, integrações, Outlook e usuário.</p>
        </div>
        <span class="exec-chip">Ambiente operacional</span>
      </div>
...
function abrirDiagnosticoInstalacaoV82128(el) {
  ...
  if (qs("tituloPagina")) qs("tituloPagina").textContent = "Saúde do Sistema";
  if (qs("subtituloPagina")) qs("subtituloPagina").textContent = "Validação pós-instalação do notebook.";
  if (qs("tituloTabela")) qs("tituloTabela").textContent = "Saúde do Sistema";
...
// ============================================================
// v8.21.49 - Central de Operação / Log Amigável
// ============================================================
function abrirServidorAssistidoV82142(el) {
  if (!usuarioPodeV82143("processar")) { alert("Acesso negado. Esta area exige perfil Supervisor ou Administrador."); return; }
  iniciarTransicaoPagina("servidor_assistido");
  paginaAtual = "servidor_assistido";
  marcarPaginaAtualClasse();
  setVisibilidadeDashboardExecutivoPremium(true);
  dadosConsolidadoAtual = [];
  ...
  if (qs("tituloPagina")) qs("tituloPagina").textContent = "Servidor";
  if (qs("subtituloPagina")) qs("subtituloPagina").textContent = "";
  if (qs("tituloTabela")) qs("tituloTabela").textContent = "Servidor";
  if (qs("statusCarga")) qs("statusCarga").textContent = "Carregando modo servidor assistido...";
  ...
  const painel = qs("dashboardExecutivoPremium");
  if (painel) painel.innerHTML = `<div class="exec-empty">Carregando servidor...</div>`;
  carregarServidorAssistidoV82142();
}

function renderPastaServidorV82142(p) {
  return `<div><strong>${valorSeguro(p.nome, "-")}</strong><span>${p.existe ? "OK" : "Pendente"}</span><small>${valorSeguro(p.caminho, "")}</small></div>`;
}

function renderChecklistServidorV82145(info) {
  const pacote = info.pacote_servidor || {};
  const checks = Array.isArray(pacote.checks) ? pacote.checks : [];
  if (!checks.length) return "";
  return `
    <section class="exec-panel process-panel-compact">
      <div class="exec-panel-title"><div><h4>Pacote Servidor Central</h4></div></div>
      <div class="process-mini-grid">
        ${checks.map(c => `<div><strong>${escapeHtml(c.nome || "-")}</strong><span>${c.ok ? "OK" : "Ajustar"}</span><small>${escapeHtml((c.detalhe || "") + (c.acao ? " " + c.acao : ""))}</small></div>`).join("")}
      </div>
      <details class="process-log-box">
        <summary>Comando e BAT oficial</summary>
        <pre>${escapeHtml(["BAT: " + (pacote.bat_oficial || ""), "Comando: " + (pacote.comando_sugerido || "")].join("\n"))}</pre>
      </details>
    </section>
  `;
}

function renderChecklistProducaoServidorV82159(info) {
  const checklist = info.checklist_producao || {};
  const itens = Array.isArray(checklist.itens) ? checklist.itens : [];
  if (!itens.length) return "";
  const classe = checklist.ok ? "success" : "warning";
  return `
    <section class="server-prod-checklist ${classe}">
      <div class="server-prod-checklist-head">
        <div>
          <span class="exec-eyebrow">Checklist de produção</span>
          <h4>${escapeHtml(checklist.status || "-")}</h4>
          <p>${escapeHtml(checklist.resumo || "")}</p>
        </div>
        <div class="server-prod-checklist-actions">
          <button class="btn secondary" type="button" onclick="abrirTesteAcessoServidorV82147()">Abrir teste</button>
          <button class="btn" type="button" onclick="copiarEnderecoServidorV82142()">Copiar endereço</button>
        </div>
      </div>
      <div class="server-prod-checklist-grid">
        ${itens.map(i => `
          <div class="${i.ok ? "ok" : "warn"}">
            <strong>${escapeHtml(i.nome || "-")}</strong>
            <span>${i.ok ? "OK" : "Atenção"}</span>
            <small>${escapeHtml(i.status || "")}</small>
            <em>${escapeHtml(i.acao || "")}</em>
          </div>
        `).join("")}
      </div>
    </section>
...
function renderServidorAssistidoV82142(info) {
  const painel = qs("dashboardExecutivoPremium") || qs("conteudoPrincipal") || document.body;
  const trava = info.trava_operacional || {};
  const pastas = info.pastas || [];
  const filaHtml = renderHistoricoFilaV82141(info.historico_fila);
  const modoClasse = info.modo_servidor ? "success" : "warning";
  const modoTexto = info.modo_servidor ? "Servidor Central" : "Local";
  const bannerServidor = info.modo_servidor
    ? `<div class="server-mode-banner success">
        <div><span>Modo atual</span><strong>OPERANDO EM MODO SERVIDOR</strong></div>
        <p>O Tempo Fechado esta aceitando acesso pela rede em ${escapeHtml(info.url_rede || "-")}.</p>
      </div>`
    : `<div class="server-mode-banner warning">
        <div><span>Modo atual</span><strong>OPERANDO EM MODO LOCAL</strong></div>
        <p>Para liberar acesso pela rede, use o botão Reiniciar agora como Servidor.</p>
      </div>`;
  const botaoServidor = info.modo_servidor
    ? `<button class="btn requires-supervisor" type="button" onclick="reiniciarComoLocalV82165()">Voltar ao modo Local</button>`
    : `<button class="btn requires-supervisor" type="button" onclick="reiniciarComoServidorV82156()">Reiniciar agora como Servidor</button>`;
  painel.innerHTML = `
    <div class="process-toolbar">
      <div class="process-toolbar-title"><h3>Servidor</h3></div>
      <div class="process-toolbar-actions">
        ${botaoServidor}
        <button class="btn" type="button" onclick="copiarEnderecoServidorV82142()">Copiar endereço</button>
        <button class="btn secondary requires-admin" type="button" onclick="validarEstruturaServidorV82142()">Validar pastas</button>
        <button class="btn secondary requires-admin" type="button" onclick="abrirPastaCentralServidorV82142()">Abrir pasta central</button>
        <button class="btn secondary requires-admin" type="button" onclick="liberarTravaProcessamentoV82140()">Liberar trava</button>
        <button class="btn secondary" type="button" onclick="carregarServidorAssistidoV82142()">Atualizar</button>
      </div>
    </div>
    ${bannerServidor}
    ${renderChecklistProducaoServidorV82159(info)}
    ${renderTravaOperacionalV82140(trava)}
    <div class="process-status-card ${modoClasse}">
      <div>
        <span class="exec-eyebrow">Modo operacional</span>
        <h4>${escapeHtml(modoTexto)}</h4>
        <p>${escapeHtml(info.ponto_pdfs_base || "")}</p>
      </div>
      <div class="process-status-meta">
        <strong>${escapeHtml(info.versao || "-")}</strong>
        <span>Host: ${escapeHtml(info.host || "-")}</span>
        <span>Porta: ${escapeHtml(String(info.porta || "-"))}</span>
        <span>Inicio: ${escapeHtml(info.iniciado_em || "-")}</span>
      </div>
    </div>
    <div class="process-mini-grid">
      <div><strong>Endereço local</strong><span>${escapeHtml(info.url_local || "-")}</span><small>Uso no servidor</small></div>
      <div><strong>Endereço na rede</strong><span id="servidorEnderecoRedeV82142">${escapeHtml(info.url_rede || "-")}</span><small>Compartilhar com usuários da rede</small></div>
      <div><strong>Estrutura</strong><span>${info.estrutura_ok ? "OK" : "Atenção"}</span><small>${pastas.filter(p => !p.existe).length} pasta(s) pendente(s)</small></div>
      <div><strong>Fila</strong><span>${trava.ativa ? "Ocupada" : "Livre"}</span><small>${escapeHtml(trava.mensagem || "Nenhum processamento em andamento.")}</small></div>
    </div>
    <section class="exec-panel process-panel-compact">
      <div class="exec-panel-title"><div><h4>Pastas do servidor</h4></div></div>
      <div class="process-mini-grid">${pastas.map(renderPastaServidorV82142).join("")}</div>
    </section>
    ${renderChecklistServidorV82145(info)}
    <section class="exec-panel process-panel-compact">
      <div class="exec-panel-title"><div><h4>Arquivos operacionais</h4></div></div>
      <div class="process-mini-grid">
        <div><strong>Trava</strong><span>JSON</span><small>${escapeHtml((info.arquivos || {}).trava || "")}</small></div>
        <div><strong>Histórico da fila</strong><span>JSONL</span><small>${escapeHtml((info.arquivos || {}).historico_fila || "")}</small></div>
        <div><strong>Histórico operacional</strong><span>JSONL</span><small>${escapeHtml((info.arquivos || {}).historico_operacao || "")}</small></div>
      </div>
    </section>
    ${filaHtml}
  `;
...
async function carregarServidorAssistidoV82142() {
  try {
    ...
    const painel = qs("dashboardExecutivoPremium");
    if (painel) painel.innerHTML = `<div class="exec-empty erro">Falha ao carregar servidor: ${escapeHtml(e.message)}</div>`;
    if (qs("statusCarga")) qs("statusCarga").textContent = "Falha ao carregar servidor.";
    return null;
  }
}
...
async function validarEstruturaServidorV82142() {
  try {
    ...
    alert("Falha ao validar estrutura: " + e.message);
  }
}

async function aguardarServidorLocalV82155(urlBase, tentativas = 24) {
  const base = String(urlBase || "http://127.0.0.1:5050").replace(/\/+$/, "");
  const teste = `${base}/saude-servidor`;
  for (let i = 0; i < tentativas; i += 1) {
    try {
      if (qs("statusCarga")) qs("statusCarga").textContent = `Aguardando servidor reiniciar... ${i + 1}/${tentativas}`;
      const resp = await fetch(teste, { cache: "no-store" });
      if (resp.ok) {
...
async function ativarModoServidorV82154() {
  return reiniciarComoServidorV82156();
}

async function reiniciarComoServidorV82156() {
  const ok = confirm(
    "O Tempo Fechado vai fechar esta instancia local e reabrir em modo Servidor.\n\n" +
    "Depois da reabertura, a guia Servidor vai mostrar claramente que o acesso pela rede esta ativo. Deseja continuar?"
  );
  if (!ok) return;
  try {
    ...
    if (json.ja_ativo) {
      alert(json.mensagem || "O modo servidor ja esta ativo.");
      carregarServidorAssistidoV82142();
      return;
    }
    alert(
      (json.mensagem || "Reinicio em modo servidor iniciado.") +
      "\n\nVou reabrir por aqui: " + local +
      (rede ? "\nEndereco de rede depois da reabertura: " + rede : "")
    );
    setTimeout(() => {
      aguardarServidorLocalV82155(local);
    }, 2500);
  } catch (e) {
    alert("Falha ao reiniciar como servidor: " + e.message);
    if (qs("statusCarga")) qs("statusCarga").textContent = "Falha ao reiniciar como servidor.";
  }
}

async function reiniciarComoLocalV82165() {
  const ok = confirm(
    "O Tempo Fechado vai fechar o modo Servidor e reabrir em modo Local neste notebook.\n\n" +
    "Após a reabertura, o acesso pela rede será desativado. Deseja continuar?"
  );
  if (!ok) return;
  try {
    ...
    alert((json.mensagem || "Reinicio em modo local iniciado.") + "\n\nVou reabrir por aqui: " + local);
    setTimeout(() => {
      aguardarServidorLocalV82155(local);
    }, 2500);
  } catch (e) {
    alert("Falha ao voltar ao modo local: " + e.message);
    if (qs("statusCarga")) qs("statusCarga").textContent = "Falha ao voltar ao modo local.";
  }
}
...
async function copiarEnderecoServidorV82142() {
  const el = qs("servidorEnderecoRecomendadoV82146") || qs("servidorEnderecoRedeV82142");
  const texto = el ? (el.textContent || "").trim() : "";
  if (!texto) {
    alert("Endereço de rede ainda não disponível.");
    return;
  }
  try {
  ...
  window.prompt("Copie o endereço do servidor:", texto);
}

async function copiarConviteServidorV82146() {
  const el = qs("servidorConviteTextoV82146");
  const texto = el ? (el.textContent || "").trim() : "";
  if (!texto) {
    alert("Convite ainda nao disponivel.");
    return;
  }
  try {
    if (navigator.clipboard && navigator.clipboard.writeText) {
...
async function copiarTesteAcessoServidorV82147() {
  const el = qs("servidorTesteAcessoV82147");
  const texto = el ? (el.textContent || "").trim() : "";
  if (!texto || texto === "-") {
    alert("Link de teste ainda nao disponivel.");
    return;
  }
  try {
    if (navigator.clipboard && navigator.clipboard.writeText) {
      await navigator.clipboard.writeText(texto);
      alert("Link de teste copiado.");
      return;
    }
  } catch (e) {}
  window.prompt("Copie o link de teste:", texto);
}

function abrirTesteAcessoServidorV82147() {
  const el = qs("servidorTesteAcessoV82147");
  const texto = el ? (el.textContent || "").trim() : "";
  if (!texto || texto === "-") {
    alert("Link de teste ainda nao disponivel.");
...
async function copiarKitServidorV82148() {
  const el = qs("servidorKitOperacionalV82148");
  const texto = el ? (el.textContent || "").trim() : "";
  if (!texto) {
    alert("Kit operacional ainda nao disponivel.");
    return;
  }
  try {
    if (navigator.clipboard && navigator.clipboard.writeText) {
      await navigator.clipboard.writeText(texto);
      alert("Kit operacional copiado.");
      return;
    }
  } catch (e) {}
  window.prompt("Copie o kit operacional:", texto);
}

function abrirSistemaServidorV82148() {
  const el = qs("servidorEnderecoRecomendadoV82146") || qs("servidorEnderecoRedeV82142");
  const texto = el ? (el.textContent || "").trim() : "";
  if (!texto || texto === "-") {
    alert("Endereco do sistema ainda nao disponivel.");
    return;
  }
  window.open(texto, "_blank", "noopener,noreferrer");
}

function abrirGovernancaServidorV82144(el) {
  if (!usuarioPodeV82143("processar")) { alert("Acesso negado. Esta area exige perfil Supervisor ou Administrador."); return; }
  iniciarTransicaoPagina("governanca_servidor");
  paginaAtual = "governanca_servidor";
  marcarPaginaAtualClasse();
  setVisibilidadeDashboardExecutivoPremium(true);
  dadosConsolidadoAtual = [];
  ...
  if (qs("tituloPagina")) qs("tituloPagina").textContent = "Governança";
  if (qs("subtituloPagina")) qs("subtituloPagina").textContent = "";
  if (qs("tituloTabela")) qs("tituloTabela").textContent = "Governança";
  if (qs("statusCarga")) qs("statusCarga").textContent = "Carregando governança operacional...";
  ...
  const painel = qs("dashboardExecutivoPremium");
  if (painel) painel.innerHTML = `<div class="exec-empty">Carregando governança...</div>`;
  carregarGovernancaServidorV82144();
}

function renderListaGovernancaV82144(itens, vazio, renderItem) {
  itens = Array.isArray(itens) ? itens : [];
  if (!itens.length) return `<div class="exec-empty">${escapeHtml(vazio || "Nenhum registro encontrado.")}</div>`;
  return `<div class="process-mini-grid">${itens.map(renderItem).join("")}</div>`;
}

function renderRiscoGovernancaV82144(r) {
  const nivel = String(r.nivel || "baixo").toLowerCase();
  const rotulo = nivel === "alto" ? "Alto" : (nivel === "medio" ? "Médio" : "Baixo");
  return `<div><strong>${escapeHtml(r.titulo || "Risco")}</strong><span>${escapeHtml(rotulo)}</span><small>${escapeHtml((r.detalhe || "") + (r.acao ? " " + r.acao : ""))}</small></div>`;
}

function renderEventoGovernancaV82144(e) {
  const nome = e.tipo || e.operacao || e.acao || "Evento";
  const status = e.status || e.evento || "-";
  const detalhe = e.mensagem || e.resumo || e.detalhe || "";
  return `<div><strong>${escapeHtml(nome)}</strong><span>${escapeHtml(status)}</span><small>${escapeHtml((e.quando || "") + " " + detalhe)}</small></div>`;
}

function renderGovernancaServidorV82144(info) {
  const painel = qs("dashboardExecutivoPremium") || qs("conteudoPrincipal") || document.body;
  const resumo = info.resumo || {};
  const perfis = resumo.perfis || {};
  const saude = info.saude || {};
  const riscos = info.riscos || [];
  const trava = info.trava_operacional || {};
  const statusTrava = trava.ativa ? "Ocupada" : "Livre";
  const modo = info.modo_operacao === "servidor" ? "Servidor" : "Local";
  painel.innerHTML = `
    <div class="process-toolbar">
      <div class="process-toolbar-title"><h3>Governança</h3></div>
      <div class="process-toolbar-actions">
        <button class="btn secondary" type="button" onclick="carregarGovernancaServidorV82144()">Atualizar</button>
        <button class="btn secondary" type="button" onclick="abrirServidorAssistidoV82142()">Servidor</button>
        <button class="btn secondary" type="button" onclick="abrirCentralOperacaoV8190()">Central</button>
      </div>
    </div>
    <div class="process-mini-grid">
      <div><strong>Modo</strong><span>${escapeHtml(modo)}</span><small>${escapeHtml(info.ponto_pdfs_base || "")}</small></div>
      <div><strong>Saúde</strong><span>${escapeHtml(String(resumo.score_saude || saude.score || 0))}</span><small>${escapeHtml(saude.nivel || "ok")}</small></div>
      <div><strong>Usuários ativos</strong><span>${escapeHtml(String(resumo.usuarios_ativos || 0))}/${escapeHtml(String(resumo.usuarios_total || 0))}</span><small>Admin ${perfis.admin || 0} · Sup ${perfis.supervisor || 0} · Op ${perfis.operador || 0} · Consulta ${perfis.consulta || 0}</small></div>
      <div><strong>Entrada</strong><span>${escapeHtml(String(resumo.pdfs_entrada || 0))} PDF(s)</span><small>Erro: ${escapeHtml(String(resumo.pdfs_erro || 0))} · Processados: ${escapeHtml(String(resumo.pdfs_processados || 0))}</small></div>
      <div><strong>Fila</strong><span>${escapeHtml(statusTrava)}</span><small>${escapeHtml(trava.mensagem || "Nenhum processamento em andamento.")}</small></div>
      <div><strong>Eventos</strong><span>${escapeHtml(String(resumo.eventos_erro || 0))} erro(s)</span><small>${escapeHtml(String(resumo.bloqueios_fila || 0))} bloqueio(s) de fila</small></div>
    </div>
    <section class="exec-panel process-panel-compact">
      <div class="exec-panel-title"><div><h4>Riscos Operacionais</h4></div></div>
      ${renderListaGovernancaV82144(riscos, "Nenhum risco operacional relevante agora.", renderRiscoGovernancaV82144)}
    </section>
    <section class="exec-panel process-panel-compact">
      <div class="exec-panel-title"><div><h4>Atividade por Usuário</h4></div></div>
      ${renderListaGovernancaV82144(info.atividade_por_usuario, "Nenhuma atividade multiusuário registrada.", x => `<div><strong>${escapeHtml(x.nome || "-")}</strong><span>${escapeHtml(String(x.qtd || 0))}</span><small>evento(s) recentes</small></div>`)}
    </section>
    <section class="exec-panel process-panel-compact">
      <div class="exec-panel-title"><div><h4>Atividade por Ação</h4></div></div>
      ${renderListaGovernancaV82144(info.atividade_por_acao, "Nenhuma ação registrada.", x => `<div><strong>${escapeHtml(x.nome || "-")}</strong><span>${escapeHtml(String(x.qtd || 0))}</span><small>ocorrência(s)</small></div>`)}
    </section>
    <section class="exec-panel process-panel-compact">
      <div class="exec-panel-title"><div><h4>Últimos Logins e Uploads</h4></div></div>
      ${renderListaGovernancaV82144([...(info.ultimos_logins || []), ...(info.ultimos_uploads || [])].slice(0, 12), "Nenhum login ou upload recente.", renderEventoGovernancaV82144)}
    </section>
    <section class="exec-panel process-panel-compact">
      <div class="exec-panel-title"><div><h4>Processamentos e Fila</h4></div></div>
      ${renderListaGovernancaV82144([...(info.eventos || []), ...(info.historico_fila || [])].slice(0, 16), "Nenhum processamento registrado.", renderEventoGovernancaV82144)}
    </section>
  `;
  aplicarPermissoesInterface();
}
...
async function carregarGovernancaServidorV82144() {
  try {
    ...
    const painel = qs("dashboardExecutivoPremium");
    if (painel) painel.innerHTML = `<div class="exec-empty erro">Falha ao carregar governança: ${escapeHtml(e.message)}</div>`;
    if (qs("statusCarga")) qs("statusCarga").textContent = "Falha ao carregar governança.";
    return null;
  }
}
...
function formatarVersaoAtivaV8215(info) {
  if (!info || !info.ok) return "Status do sistema indisponível";
  return [
    `Tempo Fechado operacional`,
    `Iniciado em: ${info.iniciado_em || "-"}`,
    `Modo: ${info.modo_operacao === "servidor" ? "Servidor" : "Local"}`,
    `Acesso: ${info.url || info.host || "-"}`,
    `Máquina: ${info.maquina || "-"}`,
    `Versão interna: ${info.versao || "-"}`
  ].join("\n");
}
...
async function carregarVersaoAtivaV8215() {
  ...
  try {
    ...
    if (badge) {
      badge.textContent = json.versao === VERSAO_ESPERADA_BADGE ? "Sistema ativo" : "Atualização pendente";
    ...
    if (badge) {
      ...
      badge.title = "Não foi possível consultar o status do sistema: " + e.message;
...
function mostrarVersaoAtivaV8215() {
  if (versaoAtivaV8215 && versaoAtivaV8215.ok) {
    ...
    alert("Não foi possível identificar o status do sistema. Tente recarregar a página ou verifique se há outra instância do Tempo Fechado aberta.");
  }
}
